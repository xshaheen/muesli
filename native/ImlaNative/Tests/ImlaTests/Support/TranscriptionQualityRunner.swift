...
// declarations with `Tests/ImlaTests/*.swift` — one level deep — so a suite parked in this
...
enum TranscriptionQualityEligibility {
    ...
    static func languageConfiguration(for backend: BackendOption) -> String {
        ...
        case "indicasr": return "pinned:\(BodhanLanguage.defaultLanguage.rawValue)"
...
/// and `MUESLI_DB_PATH` are read by exactly one place in the repository — `ImlaCLI.swift`, a
...
/// the overrides are kept for the one consumer they work for (a spawned `imla-cli`), and the one
...
enum TranscriptionQualityScratchSupportDirectory {
    ...
    static func url(root: URL, runID: String) -> URL {
        root
            .appendingPathComponent("imla-asr-harness", isDirectory: true)
            .appendingPathComponent(runID, isDirectory: true)
    }

    /// The environment a *child* `imla-cli` process would need to share this directory. Applying
    ...
    static func environmentOverrides(at directory: URL) -> [String: String] {
        [
            ...
            databasePathVariable: directory.appendingPathComponent("imla.db").path,
        ]
    }
...
struct LiveTranscriptionQualityDriver: TranscriptionQualityDriver {
    ...
    /// app that something is `ImlaController` reacting to the model selection — which no test
