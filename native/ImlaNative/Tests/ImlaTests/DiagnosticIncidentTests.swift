...
struct DiagnosticIncidentTests {
    private let metadata = DiagnosticAppMetadata(
        appVersion: "1.2.3",
        buildNumber: "456",
        bundleID: "com.xshaheen.imla.dev",
        displayName: "ImlaDev",
        macOSVersion: "15.5.0",
        architecture: "arm64"
    )
    ...
    func unknownErrorIsStrictlyRedacted() {
        ...
        #expect(incident.telemetryErrorID == "Imla.Diagnostic.dictation_transcription_failed.unclassified")
    ...
    func knownInternalErrorCodesIncludeMeaning() {
        ...
        #expect(incident.telemetryErrorID == "Imla.Diagnostic.dictation_audio_failed.microphonerecorder.3")
    ...
    func fluidAudioNotLoadedFingerprint() {
        let incident = DiagnosticIncident(
            ...
            stage: .standardDictationTranscribe,
            backendOption: .parakeetMultilingual,
            error: NSError(domain: "ImlaNativeApp.FluidAudioTranscriber.TranscriberError", code: 0),
            metadata: metadata
        )
    ...
    func nilErrorUsesAppStateCategory() {
        ...
        #expect(incident.telemetryErrorID == "Imla.Diagnostic.streaming_dictation_start_failed.streaming_controller_start_failed")
        #expect(incident.telemetryParameters["diagnostic.error_known"] == "true")
    ...
    func domainFallbackCoversSwiftEnumErrors() {
        let meaning = DiagnosticErrorCatalog.meaning(
            domain: "ImlaNativeApp.MeetingLifecycleError",
            code: "0"
        )
    ...
    func allowlistedDomainRejectsUnknownCode() {
        let incident = DiagnosticIncident(
            kind: .meetingProcessingFailed,
            stage: .meetingStopProcessing,
            error: NSError(domain: "ImlaNativeApp.MeetingLifecycleError", code: 999),
            metadata: metadata
        )
    ...
    func githubIssueURLIsPrefilled() throws {
        ...
        #expect(url.absoluteString.hasPrefix("https://github.com/xshaheen/muesli/issues/new?"))
        #expect(url.absoluteString.contains("title="))
        #expect(url.absoluteString.contains("body="))
        #expect(DiagnosticIncident.githubIssueFallbackURL.absoluteString == "https://github.com/xshaheen/muesli/issues/new")
    }
}
