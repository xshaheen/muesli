...
struct MotionTests {
    ...
    func durationsMatchSparkSurfaces() {
        #expect(ImlaTheme.Motion.morph == DictationMiniRendering.morphDuration)
        #expect(ImlaTheme.Motion.fade == DictationMiniRendering.appearanceFadeDuration)
        #expect(ImlaTheme.Motion.popIn == DictationMiniRendering.appearancePopDuration)
    }

    @Test("motion resolves normally when Reduce Motion is off")
    func normalMotionAnimates() {
        #expect(ImlaTheme.Motion.eased(0.16, reduceMotion: false) != nil)
        #expect(ImlaTheme.Motion.easedOut(0.2, reduceMotion: false) != nil)
        #expect(ImlaTheme.Motion.repeating(0.9, autoreverses: false, reduceMotion: false) != nil)
        #expect(ImlaTheme.Motion.pulsing(1.05, reduceMotion: false) != nil)
    }
    ...
    func reduceMotionRemovesAnimation() {
        ...
        #expect(ImlaTheme.Motion.eased(0.16, reduceMotion: true) == nil)
        #expect(ImlaTheme.Motion.easedOut(0.2, reduceMotion: true) == nil)
    }

    @Test("a repeating animation stops under Reduce Motion instead of running faster")
    func reduceMotionStopsRepeating() {
        #expect(ImlaTheme.Motion.repeating(0.9, autoreverses: false, reduceMotion: true) == nil)
        #expect(ImlaTheme.Motion.pulsing(1.05, reduceMotion: true) == nil)
    }
}
