...
private final class TestCKSyncPendingState: ImlaCKSyncPendingState, @unchecked Sendable {
...
private actor TestCKSyncPreparationProbe {
    private var state = ImlaCKSyncPreparationState()
...
@Suite("Imla CKSyncEngine", .serialized)
struct ImlaCKSyncEngineTests {
    ...
    private func makeCloudRecord(
        ...
        let cloud = ImlaICloudSyncEngine.syncZoneCloudRecord(from: SyncTextRecord(
            id: record.id,
            kind: record.kind,
            title: record.title,
            text: text,
    ...
    func localChangeSendsWithoutFetch() async throws {
        ...
        try await ImlaCKSyncOperation.run(
            intent: .outgoing,
    ...
    func coalescedIntentRunsBothDirectionsOnce() async throws {
        var requests = ImlaCKSyncRequestQueue()
        ...
        #expect(request == ImlaCKSyncRequest(intent: .manual, userInitiated: true))
    ...
    func failedCyclePreservesOnlyNewFollowUpIntent() throws {
        ...
        #expect(failedRequest == ImlaCKSyncRequest(intent: .outgoing, userInitiated: false))

        requests.enqueue(intent: .incoming, userInitiated: true)
        let followUpRequest = requests.consume()
        #expect(followUpRequest == ImlaCKSyncRequest(intent: .incoming, userInitiated: true))
    ...
    func preparationRecoveryErrorsAreClassified() {
        #expect(ImlaICloudSyncEngine.isICloudAccountContextError(CKError(.notAuthenticated)))
        #expect(ImlaICloudSyncEngine.isICloudAccountContextError(CKError(.permissionFailure)))
        let nestedPermission = CKError(.partialFailure, userInfo: [
            CKPartialErrorsByItemIDKey: [
                CKRecord.ID(recordName: "opaque-test-id"): CKError(.permissionFailure),
            ],
        ])
        #expect(ImlaICloudSyncEngine.isICloudAccountContextError(nestedPermission))
        let underlyingAuthentication = NSError(
            domain: NSCocoaErrorDomain,
            code: 1,
            userInfo: [NSUnderlyingErrorKey: CKError(.notAuthenticated)]
        )
        #expect(ImlaICloudSyncEngine.isICloudAccountContextError(underlyingAuthentication))
        ...
        #expect(!ImlaICloudSyncEngine.isICloudAccountContextError(overDepthLimit))
        #expect(ImlaICloudSyncEngine.isSyncZoneRecoveryError(CKError(.zoneNotFound)))
        #expect(ImlaICloudSyncEngine.isSyncZoneRecoveryError(CKError(.userDeletedZone)))
        #expect(!ImlaICloudSyncEngine.isSyncZoneRecoveryError(CKError(.networkUnavailable)))
    }
    ...
    func provenanceMissingErrorsAreBoundedAndStrict() {
        ...
        #expect(ImlaICloudSyncEngine.isMissingProvenanceRecord(allMissing))
        #expect(!ImlaICloudSyncEngine.isMissingProvenanceRecord(mixed))
    }
    ...
    func resetConflictBatchClassificationIsStrict() {
        ...
        #expect(ImlaICloudSyncEngine.isResolvedRecordConflictBatch(conflictOnly))
        #expect(ImlaICloudSyncEngine.isResolvedRecordConflictBatch(CKError(.serverRecordChanged)))
        #expect(!ImlaICloudSyncEngine.isResolvedRecordConflictBatch(mixedFailure))
        #expect(!ImlaICloudSyncEngine.isResolvedRecordConflictBatch(dependentFailureOnly))
    }
    ...
    func provenanceClassificationIsExactAndContentFree() throws {
        let zoneID = ImlaICloudSyncEngine.Schema.syncZoneID
        ...
        let first = CKRecord(
            recordType: ImlaICloudSyncEngine.Schema.textRecordType,
            recordID: firstID
        )
        let wrongType = CKRecord(recordType: "ImlaBridgeDevice", recordID: secondID)
        let unexpected = CKRecord(
            recordType: ImlaICloudSyncEngine.Schema.textRecordType,
            recordID: unexpectedID
        )

        let matches = try ImlaICloudSyncEngine.matchingProvenanceRecordNames(
    ...
    func restoredPendingChangeUsesDurableOutbox() async throws {
        ...
        let pending = CKSyncEngine.PendingRecordZoneChange.saveRecord(CKRecord.ID(
            recordName: dirty.id,
            zoneID: ImlaICloudSyncEngine.Schema.syncZoneID
        ))
        let state = TestCKSyncPendingState()
        let coordinator = ImlaCKSyncEngine(store: store)
    ...
    func localConflictWinnerUsesServerBase() async throws {
        ...
        try await coordinator.handleSentRecordChanges(
            savedRecords: [],
            failedRecordSaves: [ImlaCKSyncFailedRecordSave(record: client, error: error)],
            state: state
        )
        let batch = await coordinator.makeRecordBatch(pendingChanges: state.pendingRecordZoneChanges)
    ...
    func transientFailureRemainsPending() async throws {
        ...
        try await coordinator.handleSentRecordChanges(
            savedRecords: [],
            failedRecordSaves: [
                ImlaCKSyncFailedRecordSave(record: client, error: CKError(.networkUnavailable)),
            ],
            state: state
        )
    ...
    func accountScopeIsContentFree() {
        ...
        let firstScope = ImlaCKSyncEngine.accountScope(for: first)

        #expect(firstScope == ImlaCKSyncEngine.accountScope(for: first))
        #expect(firstScope != ImlaCKSyncEngine.accountScope(for: second))
    ...
    func accountBoundaryErrorsAreActionable() {
        let reconnectMessage = ImlaCKSyncError.legacyAccountNeedsReconnection.localizedDescription
        let mismatchMessage = ImlaCKSyncError.differentProductionAccount.localizedDescription
        ...
        #expect(!reconnectMessage.contains("ImlaCKSyncError"))
        #expect(!mismatchMessage.contains("ImlaCKSyncError"))
    }
    ...
    func cloudEntitlementContractRejectsUnspecifiedEnvironment() {
        let container = ImlaICloudSyncEngine.Schema.containerIdentifier
        #expect(ImlaICloudSyncEngine.hasRequiredCloudEntitlements(
    ...
    func accountProvenanceRequiresEveryRecord() {
        ...
        #expect(ImlaCKSyncEngine.hasExactAccountProvenance(
    ...
    func firstLocalOnlyLibraryClaimsCurrentAccount() async throws {
        ...
        let coordinator = ImlaCKSyncEngine(
            store: store,
        ...
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey)
            == Data(ImlaCKSyncEngine.accountScope(for: owner).utf8))
    ...
    func matchingUnspecifiedOwnerMigratesToProduction() async throws {
        ...
        let scope = ImlaCKSyncEngine.accountScope(for: owner)
        let migration = ImlaCKSyncLegacyScopeMigration(
    ...
    func mismatchedUnspecifiedOwnerDoesNotMigrate() async throws {
        ...
        let oldScope = ImlaCKSyncEngine.accountScope(
        ...
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey) == nil)
        #expect(try !store.hasTextRecordsNeedingSync())
        #expect(state.pendingRecordZoneChanges.isEmpty)
        ...
        do {
            ...
            #expect(error as? ImlaCKSyncError == .differentProductionAccount)
        }
        #expect(try store.cloudSyncStateData(forKey: migration.accountScopeKey)
            == Data(oldScope.utf8))
        #expect(try store.cloudSyncStateData(forKey: migration.stateKey)
        ...
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey)
            == Data(ImlaCKSyncEngine.accountScope(for: replacementOwner).utf8))
        #expect(state.pendingRecordZoneChanges == [
            .saveRecord(CKRecord.ID(
                recordName: local.id,
                zoneID: ImlaICloudSyncEngine.Schema.syncZoneID
    ...
    func explicitReconnectPreservesAndRequeuesLegacyLibrary() async throws {
        ...
        try store.saveCloudSyncStateData(
            ...
            forKey: ImlaCKSyncEngine.stateKey
        )
        ...
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey)
            == Data(ImlaCKSyncEngine.accountScope(for: currentUser).utf8))
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.stateKey) == nil)
        #expect(try store.cloudSyncStateData(forKey: migration.accountScopeKey) == nil)
        #expect(try store.cloudSyncStateData(forKey: migration.stateKey) == nil)
    ...
    func orphanedEngineStateResetIsIdempotent() async throws {
        ...
        let coordinator = ImlaCKSyncEngine(store: store, legacyScopeMigration: nil)
        #expect(try await coordinator.resetCloudSyncAccount())
        #expect(try await coordinator.resetCloudSyncAccount())
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey) == nil)
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.stateKey) == nil)
    ...
    func healthyAccountResetPreservesAndRequeuesLocalData() async throws {
        ...
        #expect(try store.claimCloudSyncAccountScope(
            ImlaCKSyncEngine.accountScope(for: owner),
            forKey: ImlaCKSyncEngine.accountScopeKey
        ))
    ...
    func accountSwitchPreservesLocalData() async throws {
        ...
        #expect(try store.claimCloudSyncAccountScope(
            ImlaCKSyncEngine.accountScope(for: originalOwner),
            forKey: ImlaCKSyncEngine.accountScopeKey
        ))
        let migration = ImlaCKSyncLegacyScopeMigration(
        ...
        } catch let error as ImlaCKSyncError {
            #expect(error == .differentProductionAccount)
        }
        #expect(try store.cloudSyncStateData(forKey: ImlaCKSyncEngine.accountScopeKey)
            == Data(ImlaCKSyncEngine.accountScope(for: originalOwner).utf8))
    ...
    func migratedLibraryStillRepairsZoneRecreationDuringPreparation() async throws {
        ...
        let suiteName = "ImlaCKSyncEngineTests.\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
        defaults.set(true, forKey: ImlaICloudSyncEngine.Schema.migratedDefaultZoneKey)
        var migrationRuns = 0

        let recreated = try await ImlaICloudSyncEngine.prepareForCKSyncEngine(
            store: store,
            ensureZone: { true },
            migrate: {
                guard !defaults.bool(
                    forKey: ImlaICloudSyncEngine.Schema.migratedDefaultZoneKey
                ) else { return }
                migrationRuns += 1
            }
        )
