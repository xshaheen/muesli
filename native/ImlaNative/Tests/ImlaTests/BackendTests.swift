...
struct Nemotron35ModelStoreTests {
    ...
    func sharedCachePath() {
        #expect(Nemotron35ModelStore.cacheRelativePath == ".cache/imla/models/nemotron35-multilingual-2240ms")
...
struct Gemma4LiteRTTranscriberTests {
    ...
    func gemma4ModelStoreEnvOverride() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-gemma4-store-test-\(UUID().uuidString)", isDirectory: true)
    ...
    func gemma4ModelStoreRejectsDirectoryOverride() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-gemma4-directory-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func gemma4DeleteRespectsModelOverride() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-gemma4-delete-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func gemma4DefaultPathsUseManagedCache() {
        #expect(Gemma4LiteRTModelStore.cacheRelativePath == ".cache/imla/models/gemma-4-e2b-litert-lm")
        #expect(Gemma4LiteRTModelStore.managedModelURL().path.hasSuffix("/.cache/imla/models/gemma-4-e2b-litert-lm/\(Gemma4LiteRTModelStore.modelFilename)"))
        #expect(Gemma4LiteRTModelStore.managedLiteRTCacheDirectory().path.hasSuffix("/.cache/imla/models/gemma-4-e2b-litert-lm/litert-cache"))
        #expect(Gemma4LiteRTModelStore.managedModelURL(for: .e4b).path.hasSuffix("/.cache/imla/models/gemma-4-e4b-litert-lm/\(Gemma4LiteRTModel.e4b.filename)"))
        #expect(Gemma4LiteRTModelStore.managedLiteRTCacheDirectory(for: .e4b).path.hasSuffix("/.cache/imla/models/gemma-4-e4b-litert-lm/litert-cache"))
    }
    ...
    func gemma4ManagedDownloadValidationRejectsTinyFiles() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-gemma4-small-\(UUID().uuidString).litertlm")
    ...
    func gemma4ManagedDownloadValidationRejectsDirectories() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-gemma4-download-directory-\(UUID().uuidString)", isDirectory: true)
    ...
    func gemma4UserMessageContainsPromptThenAudio() throws {
        let wavURL = URL(fileURLWithPath: "/tmp/imla-gemma4-sample.wav")
    ...
    func gemma4RejectsLongAudio() throws {
        ...
        let wavURL = try WavWriter.writeTemporaryWAV(samples: samples, directoryName: "imla-gemma4-duration-test")
