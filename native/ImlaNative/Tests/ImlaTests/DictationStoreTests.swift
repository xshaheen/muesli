...
struct DictationStoreTests {
    ...
    func deletedSyncCloudRecordsOmitTextContentFields() throws {
        ...
        let cloud = ImlaICloudSyncEngine.syncZoneCloudRecord(from: record)
    ...
    func syncCloudRecordCanUpdateExistingServerRecord() throws {
        ...
        let recordID = CKRecord.ID(
            ...
            zoneID: CKRecordZone.ID(zoneName: "ImlaSyncZone", ownerName: CKCurrentUserDefaultName)
        )
        let baseRecord = CKRecord(recordType: "ImlaTextRecord", recordID: recordID)
        baseRecord["text"] = "Server text" as NSString
        baseRecord["updatedAt"] = updatedAt.addingTimeInterval(-60) as NSDate

        let cloud = ImlaICloudSyncEngine.syncZoneCloudRecord(
            from: SyncTextRecord(
    ...
    func syncCloudRecordRestoresPersistedSystemFields() throws {
        ...
        let systemFields = try #require(ImlaICloudSyncEngine.encodedSystemFields(for: original))
        ...
        #expect(cloud.recordID == recordID)
        #expect(cloud.recordType == ImlaICloudSyncEngine.Schema.textRecordType)
    ...
    func syncCloudRecordRejectsForeignZoneSystemFields() throws {
        ...
        let legacySystemFields = try #require(
            ImlaICloudSyncEngine.encodedSystemFields(for: legacyRecord)
        )
        ...
        #expect(cloud.recordID.zoneID == ImlaICloudSyncEngine.Schema.syncZoneID)
    ...
    func dirtyUploadResolutionAppliesNewerFetchedRemote() throws {
        ...
        #expect(ImlaICloudSyncEngine.shouldApplyFetchedRemoteBeforeDirtyUpload(
            local: local,
            remote: remote,
    ...
    func cleanedMeetingStateRoundTripsThroughCloudKit() throws {
        ...
        let firstCloud = ImlaICloudSyncEngine.syncZoneCloudRecord(from: firstOutbound)
        ...
        let firstRemote = try #require(ImlaICloudSyncEngine.syncTextRecord(from: firstCloud))
        ...
        let secondCloud = ImlaICloudSyncEngine.syncZoneCloudRecord(from: secondOutbound)
        let secondRemote = try #require(ImlaICloudSyncEngine.syncTextRecord(from: secondCloud))
        ...
        let legacyRemote = try #require(ImlaICloudSyncEngine.syncTextRecord(from: secondCloud))
    ...
    func cloudKitExpiredChangeTokenErrorsAreDetected() {
        #expect(ImlaICloudSyncEngine.isChangeTokenExpired(CKError(.changeTokenExpired)))
        #expect(!ImlaICloudSyncEngine.isChangeTokenExpired(CKError(.networkUnavailable)))
    }
    ...
    func cloudKitServerRecordChangedErrorsAreDetected() {
        ...
        #expect(ImlaICloudSyncEngine.isServerRecordChangedError(CKError(.serverRecordChanged)))
        #expect(ImlaICloudSyncEngine.isServerRecordChangedError(partialFailure))
        #expect(!ImlaICloudSyncEngine.isServerRecordChangedError(CKError(.networkUnavailable)))
    }
    ...
    func treeOrderedFoldersProducesCorrectOrder() {
        ...
        let ordered = ImlaController.treeOrderedFolders(folders, order: [1, 2, 3, 4, 5])
    ...
    func treeOrderedFoldersHandlesOrphans() {
        ...
        let ordered = ImlaController.treeOrderedFolders(folders, order: [1, 2, 3])
    ...
    func treeOrderedFoldersIncludesClosedCyclesOnce() {
        ...
        let ordered = ImlaController.treeOrderedFolders(folders, order: [3, 4, 1, 2])
        let ids = ordered.map(\.id)
        #expect(ids == [3, 4, 1, 2])
    ...
    func migrationAddsCleanupColumns() throws {
        ...
        #expect(meeting.visualContext == nil)
