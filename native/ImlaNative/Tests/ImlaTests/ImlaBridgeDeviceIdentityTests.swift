...
@Suite("ImlaBridgeDeviceIdentity", .serialized)
struct ImlaBridgeDeviceIdentityTests {
    ...
    private func makeDefaults() throws -> (UserDefaults, String) {
        let suiteName = "com.xshaheen.imla.tests.bridge-device-identity.\(UUID().uuidString)"
    ...
    private func bridgeRecord(
        ...
        let record = CKRecord(recordType: "ImlaBridgeDevice")
        if let deviceID {
    ...
    func shouldRefreshReturnsTrueWithNoTimestamp() throws {
        ...
        #expect(ImlaBridgeDeviceIdentity.shouldRefresh(
            defaults: defaults,
    ...
    func shouldRefreshUsesShortIntervalBeforeRemoteDeviceIsKnown() throws {
        ...
        ImlaBridgeDeviceIdentity.markRefreshed(defaults: defaults, at: now)
    ...
    func failedRefreshUsesShortRetryBackoffInsteadOfSuccessThrottle() throws {
        ...
        ImlaBridgeDeviceIdentity.markRefreshed(defaults: defaults, at: now.addingTimeInterval(-10))
        ImlaBridgeDeviceIdentity.markRefreshFailed(defaults: defaults, at: now)
    ...
    func updateRemoteDevicesWithEmptyRecordsClearsRemoteKeys() throws {
        ...
        ImlaBridgeDeviceIdentity.updateRemoteDevices(from: [], defaults: defaults)
    ...
    func clearRemoteDevicePreservesLocalIdentity() throws {
        ...
        ImlaBridgeDeviceIdentity.clearRemoteDevice(defaults: defaults)
    ...
    func updateRemoteDevicesPicksMostRecentNonLocalRecord() throws {
        ...
        ImlaBridgeDeviceIdentity.updateRemoteDevices(
    ...
    func snapshotReturnsNilWhenRequiredFieldsAreMissing() {
        #expect(ImlaBridgeDeviceIdentity.snapshot(from: bridgeRecord(deviceID: nil)) == nil)
        #expect(ImlaBridgeDeviceIdentity.snapshot(from: bridgeRecord(platform: nil)) == nil)
        #expect(ImlaBridgeDeviceIdentity.snapshot(from: bridgeRecord(deviceName: nil)) == nil)
        #expect(ImlaBridgeDeviceIdentity.snapshot(from: bridgeRecord(lastSeenAt: nil)) == nil)
    }
    ...
    func localPersistsUUIDAcrossCallsForSameDefaultsSuite() throws {
        ...
        let first = ImlaBridgeDeviceIdentity.local(defaults: defaults)
        let second = ImlaBridgeDeviceIdentity.local(defaults: defaults)
...
@Suite("ImlaBridgeDeviceRefreshPolicy")
struct ImlaBridgeDeviceRefreshPolicyTests {
    @Test("user initiated sync forces bridge device refresh")
    func userInitiatedSyncForcesBridgeDeviceRefresh() {
        #expect(ImlaBridgeDeviceRefreshPolicy.shouldForceRefresh(
            userInitiated: true,
