...
struct InkLadderTests {
    ...
    func hairlineNormalAlpha() {
        #expect(ImlaTheme.hairlineAlpha(isDark: true, increaseContrast: false) == 0.07)
        #expect(ImlaTheme.hairlineAlpha(isDark: false, increaseContrast: false) == 0.08)
    }

    @Test("Increase Contrast strengthens the hairline in both appearances")
    func hairlineHighContrastAlpha() {
        // ImlaTheme had no Increase Contrast path at all before this: the hairline stayed
        // at 7-8% however the setting was configured.
        #expect(ImlaTheme.hairlineAlpha(isDark: true, increaseContrast: true) == 0.80)
        #expect(ImlaTheme.hairlineAlpha(isDark: false, increaseContrast: true) == 0.80)
    }

    @Test("the disabled ink step sits below tertiary in both appearances")
    func disabledSitsBelowTertiary() {
        #expect(ImlaTheme.Ink.disabledDarkAlpha < ImlaTheme.Ink.tertiaryDarkAlpha)
        #expect(ImlaTheme.Ink.disabledLightAlpha < ImlaTheme.Ink.tertiaryLightAlpha)
    }
}
...
struct CornerGeometryTests {
    ...
    func radiusScaleIsOrdered() {
        #expect(ImlaTheme.cornerChip < ImlaTheme.cornerSmall)
        #expect(ImlaTheme.cornerSmall < ImlaTheme.cornerMedium)
        #expect(ImlaTheme.cornerMedium < ImlaTheme.cornerLarge)
        #expect(ImlaTheme.cornerLarge < ImlaTheme.cornerXL)
    }
