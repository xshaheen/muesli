...
struct PasteControllerTests {
    ...
    private func makePasteboard() -> NSPasteboard {
        let name = NSPasteboard.Name("com.xshaheen.imla.tests.PasteController.\(UUID().uuidString)")
...
struct AutoEnterDeliveryTests {
    ...
    /// clipboard Imla did not stage.
    @Test("the dispatch callback reports whether Imla still owned the staged clipboard")
    ...
    func markedReturnIgnoredByHotkeyMonitor() throws {
        ...
        ImlaSyntheticKeyboardEvent.mark(event)

        #expect(
            event.getIntegerValueField(.eventSourceUserData)
                == ImlaSyntheticKeyboardEvent.userDataMarker
        )
    }
}
