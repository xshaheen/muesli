...
struct MeetingsNavigationTests {
    ...
    private func makeSupportDirectory() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-nav-support-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func discardConfirmationResolutionMapping() {
        #expect(
            ImlaController.discardResolution(
    ...
    func discardAppliesOnlyToItsOwnRecording() {
        ...
        #expect(ImlaController.shouldApplyDiscard(alertOwnerID: owner, activeOwnerID: owner))
        #expect(!ImlaController.shouldApplyDiscard(alertOwnerID: owner, activeOwnerID: replacement))
        #expect(!ImlaController.shouldApplyDiscard(alertOwnerID: owner, activeOwnerID: nil))
        // The menu path raises the alert without a panel behind it, so it is not scoped.
        #expect(ImlaController.shouldApplyDiscard(alertOwnerID: nil, activeOwnerID: replacement))
        #expect(ImlaController.shouldApplyDiscard(alertOwnerID: nil, activeOwnerID: nil))
    }
    ...
    func startupMigrationPreservesSharedLegacyRecordingOwners() throws {
        ...
        defer { try? FileManager.default.removeItem(at: supportDirectory) }
        try FileManager.default.createDirectory(at: supportDirectory, withIntermediateDirectories: true)
        let database = supportDirectory.appendingPathComponent("imla.sqlite")
        ...
        try ImlaController.migrateLegacyMeetingRecordings(
    ...
    func resummaryContextStripsWrittenNotesSection() {
        ...
        let context = ImlaController.notesContextForResummary(meeting)
    ...
    func legacyOpenRouterCredentialShowsModels() {
        ...
        controller.updateConfig { $0.openRouterAPIKey = " sk-or-v1-legacy " } // gitleaks:allow
    ...
    func failedOpenRouterDisconnectPreservesSelections() throws {
        ...
        let configStore = ConfigStore(supportDirectory: supportDirectory)
        let credentialStore = OpenRouterCredentialStore(supportDirectory: supportDirectory)
        try credentialStore.save(OpenRouterCredential(apiKey: "sk-or-v1-retained", userID: nil)) // gitleaks:allow
        let openRouterAuth = OpenRouterAuthManager(
            credentialStore: credentialStore,
            loadData: { _ in throw URLError(.unsupportedURL) },
            openURL: { _ in false },
