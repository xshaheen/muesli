...
struct SemanticColorTests {
    ...
    func semanticTokensAreDistinct() {
        let all = [
            ImlaTheme.recordingHex,
            ImlaTheme.transcribingHex,
            ImlaTheme.dangerHex,
            ImlaTheme.successHex,
        ]
    ...
    func semanticValuesMatchSpark() {
        #expect(ImlaTheme.recordingHex == DictationMiniPalette.accentHex)
        #expect(ImlaTheme.transcribingHex == DictationMiniPalette.accentHighlightHex)
        #expect(ImlaTheme.dangerHex == DictationMiniPalette.failureHex)
        #expect(ImlaTheme.successHex == DictationMiniPalette.successHex)
    }
    ...
    func failedMeetingUsesDanger() {
        ...
        // running one.
        #expect(MeetingStatus.failed.displayColor == ImlaTheme.danger)
        #expect(MeetingStatus.failed.displayColor != ImlaTheme.transcribing)
    }

    @Test("a recording meeting reads as recording")
    func recordingMeetingUsesRecording() {
        #expect(MeetingStatus.recording.displayColor == ImlaTheme.recording)
    }

    @Test("a completed meeting reads as success")
    func completedMeetingUsesSuccess() {
        #expect(MeetingStatus.completed.displayColor == ImlaTheme.success)
    }
}
...
struct SemanticStateAccentIndependenceTests {
    ...
    func processingIgnoresAccentPreset() {
        let original = ImlaTheme.accentOverrideHex
        defer { ImlaTheme.accentOverrideHex = original }

        ImlaTheme.accentOverrideHex = nil
        let withDefaultAccent = MeetingStatus.processing.displayColor

        ImlaTheme.accentOverrideHex = "8b5cf6"
        ...
        #expect(withPurpleAccent == ImlaTheme.transcribing)
    }
}
...
struct SemanticColorSourceTests {
    ...
    func destructiveControlsDoNotUseRecording() throws {
        for fileName in sweptViewFiles {
            ...
            #expect(
                !source.contains("isDestructive ? ImlaTheme.recording"),
