...
struct DictationCleanupPolicyTests {
    ...
    func customInstructionsSitBetweenStyleAndVocabulary() throws {
        ...
        let prompt = DictationCleanupPromptComposer.systemPrompt(
            ...
            customWords: [CustomWord(word: "muesli", replacement: "Imla")]
        )
    ...
    func emptyCustomInstructionsKeepBytes() {
        let words = [CustomWord(word: "muesli", replacement: "Imla")]
    ...
    func preloadMatchesSessionWithoutAdaptiveStyles() throws {
        ...
        config.customWords = [CustomWord(word: "muesli", replacement: "Imla")]
    ...
    func fallbackOutcomesAndFinalOrdering() {
        ...
        let words = [CustomWord(word: "museli", replacement: "Imla")]
        ...
        for (attempt, expectedOutcome) in attempts {
            ...
            #expect(result.text == "Send this to Imla")
        }
    }
    ...
    func deadlineFallbackPreservesRawTranscript() {
        let result = DictationCleanupFinalizer.finalize(
            ...
            customWords: [CustomWord(word: "museli", replacement: "Imla")],
            provenance: nil,
            fallbackResult: original
        )

        #expect(result.cleanupOutcome == .fallbackDeadline)
        #expect(result.text == "Um send this to Imla")
...
struct Qwen3PostProcessingOutputCleanerTests {
    ...
    func hostedCleanupSanitizerPreservesLabelsAndQuotes() {
        ...
        #expect(cleaned.contains(#"Subject: "Imla launch notes""#))
        #expect(cleaned.contains(#"Body: Ask Priyanka to review the "AI Models" settings copy."#))
        #expect(!Qwen3PostProcessorOutputCleaner.shouldFallbackToInput(
            cleaned: cleaned,
            input: #"Subject quote Imla launch notes body ask Priyanka to review the quote AI Models quote settings copy"#
        ))
    }
}
...
struct DictationRepairCompositionTests {
    ...
    @Test("an overriding mode still receives the repair block")
    func overridingModeKeepsRepair() throws {
        // Repair is Imla's correctness rule, not a preference, so "use these
        // instructions instead of the default ones" must not switch it off.
        let config = try bilingualConfig()
        let mode = DictationModeSelection(
            modeID: "chat",
            modeName: "Chat",
    ...
    @Test("a mode's instructions and the repair block coexist in order")
    func modeAndRepairCoexist() throws {
        var config = try bilingualConfig()
        config.customInstructions = "Use British English."
        let mode = DictationModeSelection(
            modeID: "chat",
        ...
        let repair = try #require(prompt.range(of: MixedLanguageRepairPrompt.openingTag))
        // Repair last, so it outranks both the global and the mode preferences.
        #expect(modeBlock.lowerBound < repair.lowerBound)
    }
