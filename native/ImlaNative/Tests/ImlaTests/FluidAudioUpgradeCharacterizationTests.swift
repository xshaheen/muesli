...
/// repository or revision, Imla goes on offering it under the same label, and a user
...
struct FluidAudioUpgradeCharacterizationTests {
    ...
    /// The one cross-boundary contract in this suite: Imla hardcodes each repository
    ...
    /// changes, this breaks, and the upgrade stops rather than shipping a Imla that
    /// downloads from a repository FluidAudio no longer serves.
    ///
    /// Everything below this case compares Imla values to Imla values. That is worth
    ...
    @Test("Imla's repository identifiers still match FluidAudio's own")
    ...
    /// Every managed model's repository and revision, frozen. Imla values on both
    ...
    /// `NemotronRNNTConfig` is **Imla's own** type (`NemotronRNNTEngine.swift`), not
    ...
    ///
    /// It still earns a place pinning the geometry Imla feeds the model (2240 ms at
