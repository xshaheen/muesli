...
struct AsrVocabularyPromptTests {
    ...
    func joinsTerms() {
        let prompt = AsrVocabularyPrompt.build(customWords: [word("Kubernetes"), word("Imla"), word("Parakeet")])
        #expect(prompt?.text == "Kubernetes, Imla, Parakeet")
    ...
    func capsSingleOversizedTerm() {
        ...
        let prompt = AsrVocabularyPrompt.build(customWords: [word("Imla"), word(long)])
        #expect(prompt?.text == "Imla")
    }
