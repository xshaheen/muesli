...
public final class DictationStore {
    ...
    public init() {
        self.databaseURL = ImlaPaths.defaultDatabaseURL()
    ...
    private func normalizeLegacySchema(db: OpaquePointer?) throws {
        ...
        -- Contacts identifiers are never sent through Imla's sync layer.
    ...
    private func removeCachedInsightsRecord(kind: String, id: Int64, db: OpaquePointer?) throws {
        ...
        do {
            ...
            throw NSError(domain: "ImlaInsightsCache", code: 3, userInfo: [NSLocalizedDescriptionKey: "Subtracting daily contribution: \(error.localizedDescription)"])
        }
        for pair in InsightsContributionCodec.decode(blob) {
            do {
                ...
                throw NSError(domain: "ImlaInsightsCache", code: 4, userInfo: [NSLocalizedDescriptionKey: "Subtracting token \(pair.tokenID): \(error.localizedDescription)"])
            }
        }
    ...
    private func dirtyMeetingTextRecords(
        ...
        SELECT m.cloud_record_name, m.title, m.raw_transcript, m.formatted_notes, m.manual_notes,
               ...
               m.updated_at, m.deleted_at, m.cloud_change_tag,
               COALESCE(m.follow_up_to_record_name, predecessor.cloud_record_name),
               m.cleaned_transcript, m.notes_source, m.cloud_system_fields
        FROM meetings AS m
        LEFT JOIN meetings AS predecessor ON predecessor.id = m.follow_up_to_id
    ...
    public func markTextRecordSynced(
        ...
        sqlite3_bind_text(statement, 4, (recordName as NSString).utf8String, -1, sqliteTransient)
    ...
    private func makeSyncMeetingRecord(_ statement: OpaquePointer?) -> SyncTextRecord? {
        ...
        return SyncTextRecord(
            ...
            cloudSystemFields: optionalDataColumn(statement, index: 16),
    ...
    private func upsertSyncedMeeting(_ record: SyncTextRecord, db: OpaquePointer?) throws -> Bool {
        ...
        bindOptionalBlob(record.cloudSystemFields, at: 19, statement: statement)
        sqlite3_bind_double(statement, 20, Date().timeIntervalSince1970)
        sqlite3_bind_int(statement, 21, cleanedTranscript == nil ? 0 : 1)
        sqlite3_bind_int(statement, 22, notesSource == nil ? 0 : 1)
        guard sqlite3_step(statement) == SQLITE_DONE else {
            throw lastError(db)
        }
