...
/// A third-party ASR model whose transport is owned by Imla.
public struct ManagedASRModelPlan: Sendable {
    ...
    private static let completionMarkerName = ".imla-managed-model-complete.json"
    ...
    /// Optional immutable Imla mirror used before Hugging Face discovery.
    public let mirror: ImlaModelMirror?
    ...
    /// created by a Imla version that predates managed completion markers.
...
public enum ManagedASRModelDownloader {
    ...
    public static func downloadIfNeeded(
        ...
        mirrorResolver: ImlaModelMirrorManifestResolver = .shared,
    ...
    private static func performDownload(
        ...
        mirrorResolver: ImlaModelMirrorManifestResolver,
        ...
        if let mirror = plan.mirror {
            do {
                report("Checking Imla model mirror...")
                ...
                report("Imla mirror unavailable; trying Hugging Face...")
            }
        } else {
    ...
    private static func moveCacheAside(_ cacheDirectory: URL) throws -> URL? {
        ...
        let backup = cacheDirectory.deletingLastPathComponent().appendingPathComponent(
            ".\(cacheDirectory.lastPathComponent).imla-mirror-fallback-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    private static func recoverInterruptedFallbackCache(for plan: ManagedASRModelPlan) throws {
        ...
        let prefix = ".\(cacheDirectory.lastPathComponent).imla-mirror-fallback-"
