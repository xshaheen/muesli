...
struct CLIContext {
    ...
    init(dbPath: String?, supportDir: String?) {
        ...
        if let supportDir, !supportDir.isEmpty {
            ...
            self.databaseURL = self.supportDirectory.appendingPathComponent("imla.db")
            return
        }
        ...
        self.supportDirectory = ImlaPaths.defaultSupportDirectoryURL()
        self.databaseURL = self.supportDirectory.appendingPathComponent("imla.db")
    }
...
func appBundlePath() -> String? {
    ...
    let defaultPath = "/Applications/Imla.app"
...
func ensureDatabaseAvailable(_ context: CLIContext, command: String) throws {
    guard context.store.databaseExists else {
        throw CLIError.databaseUnavailable(
            "No Imla database exists at the resolved path.",
            fix: "Launch Imla once or pass --db-path/--support-dir to point at the correct data directory."
        )
    }
}
...
func migrationWarnings(_ context: CLIContext) -> [String] {
    do {
        ...
        return ["Schema migration failed: \(error.localizedDescription). "
                + "Results may be missing fields added by newer Imla versions."]
    }
}
...
struct GlobalOptions: ParsableArguments {
    @Option(name: .long, help: "Override the absolute path to imla.db.")
    var dbPath: String?

    @Option(name: .long, help: "Override the Imla support directory. The CLI will look for imla.db inside it.")
...
@main
struct ImlaCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "imla-cli",
        abstract: "Agent-friendly CLI for local Imla meetings and dictations.",
    ...
    static func exit(withError error: Error? = nil) -> Never {
        ...
        if let validation = error as? ValidationError {
            emitFailure(
                ...
                error: ErrorBody(code: "invalid_input", message: validation.message, fix: "Run `imla-cli spec` for valid usage."),
                dbPath: nil
            )
            Foundation.exit(4)
        }
        ...
        emitFailure(
            ...
            error: ErrorBody(
                ...
                fix: "Run `imla-cli spec` for valid usage."
            ),
            dbPath: nil
        )
    ...
    func run() throws {
        ...
        emitSuccess(command: "imla-cli", data: payload, dbPath: CLIContext(options: .init()).databaseURL)
    }

    static func specPayload() -> CommandSpecPayload {
        CommandSpecPayload(commands: [
            .init(name: "spec", usage: "imla-cli spec", summary: "Dump the command tree and CLI schema metadata.", examples: ["imla-cli spec"]),
            .init(name: "info", usage: "imla-cli info [--db-path <path>] [--support-dir <dir>]", summary: "Show resolved support and database paths.", examples: ["imla-cli info", "imla-cli info --support-dir ~/Library/Application\\ Support/Imla"]),
            .init(name: "transcribe", usage: "imla-cli transcribe <file> [--format text|json|markdown] [--model parakeet-v3|parakeet-v2|parakeet-unified|parakeet-eou-320ms|sensevoice|nemotron35|whisper-tiny|whisper-tiny-english|whisper-small|whisper-small-english|whisper-medium-english|whisper-large-turbo] [--summarize] [--save-meeting] [--title <title>] [--output <path>] [--dictionary <path>]", summary: "Transcribe a local audio file or voice note (\(ImportableAudioFormat.sortedExtensions.joined(separator: ", "))) with Imla's bundled local ASR models.", examples: ["imla-cli transcribe call.mp3", "imla-cli transcribe PTT-20260921-WA0001.opus --model whisper-large-turbo --language ar", "imla-cli transcribe call.m4a --format json", "imla-cli transcribe call.wav --model nemotron35 --dictionary dictionary.json"]),
            .init(name: "meetings list", usage: "imla-cli meetings list [--limit <n>] [--folder-id <id>]", summary: "List recent meetings.", examples: ["imla-cli meetings list --limit 5", "imla-cli meetings list --folder-id 2"]),
            .init(name: "meetings get", usage: "imla-cli meetings get <id>", summary: "Return a full meeting record.", examples: ["imla-cli meetings get 42"]),
            .init(name: "meetings update-notes", usage: "imla-cli meetings update-notes <id> (--stdin | --file <path>)", summary: "Replace stored meeting notes only.", examples: ["imla-cli meetings update-notes 42 --file notes.md", "cat notes.md | imla-cli meetings update-notes 42 --stdin"]),
            .init(name: "dictations list", usage: "imla-cli dictations list [--limit <n>]", summary: "List recent dictations.", examples: ["imla-cli dictations list --limit 10"]),
            .init(name: "dictations get", usage: "imla-cli dictations get <id>", summary: "Return a full dictation record.", examples: ["imla-cli dictations get 7"]),
        ])
    }
}
...
struct SpecCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        emitSuccess(command: "imla-cli spec", data: ImlaCLI.specPayload(), dbPath: context.databaseURL)
    }
}
...
struct MeetingsCommand: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "meetings", abstract: "Inspect and update Imla meetings.", subcommands: [MeetingsListCommand.self, MeetingsGetCommand.self, MeetingsUpdateNotesCommand.self])
}
...
struct MeetingsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "imla-cli meetings list", data: [MeetingListRow](), dbPath: context.databaseURL, warnings: ["No Imla database exists at the resolved path."])
            return
        }
        ...
        emitSuccess(command: "imla-cli meetings list", data: rows, dbPath: context.databaseURL, warnings: warnings)
    }
}
...
struct MeetingsGetCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "imla-cli meetings get")
        let (found, warnings) = try withMigration(context) { try context.store.meeting(id: id) }
        guard let meeting = found else {
            throw CLIError.notFound("No meeting exists with id \(id).", fix: "Run `imla-cli meetings list` to find a valid ID.")
        }
        emitSuccess(command: "imla-cli meetings get", data: MeetingDetailPayload(meeting), dbPath: context.databaseURL, warnings: warnings)
    }
}
...
struct MeetingsUpdateNotesCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "imla-cli meetings update-notes")
        ...
        ImlaNotifications.postDataDidChange()
        ...
        emitSuccess(command: "imla-cli meetings update-notes", data: MeetingDetailPayload(updated), dbPath: context.databaseURL, warnings: warnings)
    }
}

struct DictationsCommand: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "dictations", abstract: "Inspect Imla dictations.", subcommands: [DictationsListCommand.self, DictationsGetCommand.self])
}
...
struct DictationsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "imla-cli dictations list", data: [DictationListRow](), dbPath: context.databaseURL, warnings: ["No Imla database exists at the resolved path."])
            return
        }
        let (rows, warnings) = try withMigration(context) {
            try context.store.recentDictations(limit: limit).map(DictationListRow.init)
        }
        emitSuccess(command: "imla-cli dictations list", data: rows, dbPath: context.databaseURL, warnings: warnings)
    }
}
...
struct DictationsGetCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "imla-cli dictations get")
        let (found, warnings) = try withMigration(context) { try context.store.dictation(id: id) }
        guard let dictation = found else {
            throw CLIError.notFound("No dictation exists with id \(id).", fix: "Run `imla-cli dictations list` to find a valid ID.")
        }
        emitSuccess(command: "imla-cli dictations get", data: DictationDetailPayload(dictation), dbPath: context.databaseURL, warnings: warnings)
    }
}
