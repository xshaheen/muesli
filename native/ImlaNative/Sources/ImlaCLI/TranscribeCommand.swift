...
enum TranscribeModel: String, CaseIterable, ExpressibleByArgument, Encodable {
    ...
    var asrModelVersion: AsrModelVersion? {
        ...
        case .parakeetUnified, .parakeetEou320ms, .senseVoice, .nemotron35,
    ...
    var whisperKitModelName: String? {
        ...
        case .parakeetV3, .parakeetV2, .parakeetUnified, .parakeetEou320ms, .senseVoice, .nemotron35:
            return nil
        }
    }
...
struct TranscribeCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Transcribe a local audio file with Imla's bundled local ASR models."
    )
    ...
    @Option(
        name: .long,
        help: "Transcription model: parakeet-v3, parakeet-v2, parakeet-unified, parakeet-eou-320ms (streaming), sensevoice, nemotron35, whisper-tiny, whisper-tiny-english, whisper-small, whisper-small-english, whisper-medium-english, or whisper-large-turbo.",
    ...
    var language = "auto"
    @Flag(name: .long, help: "Generate meeting notes using the configured Imla summary backend when available.")
    var summarize = false
    @Flag(name: .long, help: "Save the transcript as an imported Imla meeting.")
...
extension TranscribeJSONPayload {
    init(_ result: ImlaAudioTranscriptionResult) {
        self.init(
...
struct ImlaAudioTranscriptionRequest {
    let sourceURL: URL
    ...
    /// correction step Imla applies to dictations, so this measures "what if the
    /// dictionary were enabled" against exactly the shipped implementation.
    var dictionaryURL: URL? = nil
}

struct ImlaAudioTranscriptionResult {
    let title: String
    let transcript: String
    let summary: String?
    let durationSeconds: Double
...
struct ImlaAudioTranscriptionPipeline {
    ...
    init(
        audioPreparer: AudioPreparing = ImlaAudioFilePreparer(),
        transcriber: AudioTranscribing = RoutingAudioTranscriber(),
        summarizer: MeetingSummarizing = ConfiguredCLIMeetingSummarizer(),
        dataChangePoster: @escaping () -> Void = ImlaNotifications.postDataDidChange
    ) {
    ...
    func run(request: ImlaAudioTranscriptionRequest, context: CLIContext) async throws -> ImlaAudioTranscriptionResult {
        ...
        fputs("[imla-cli] preparing audio...\n", stderr)
        ...
        return ImlaAudioTranscriptionResult(
            title: title,
    ...
    static func rawTranscriptNotes(transcript: String, title: String, summaryRequested: Bool, warnings: [String]) -> String {
        ...
        if summaryRequested {
            ...
            if warnings.isEmpty {
                sections.append("Imla could not generate structured notes from the configured summary backend.")
            } else {
...
struct ImlaAudioFilePreparer: AudioPreparing {
    ...
    private func temporaryWAVURL() throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("imla-cli-import", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
    ...
    private func decodeAssetReaderToTemporaryWAV(sourceURL: URL) async throws -> (wavURL: URL, sampleCount: Int) {
        ...
        let wavURL = try CLIWavWriter.temporaryWAVURL(directoryName: "imla-cli-import")
        do {
    ...
    private func load(model: TranscribeModel, progress: @escaping (String) -> Void) async throws {
        ...
        guard let asrModelVersion = model.asrModelVersion else {
            throw CLIError.invalidInput(
                ...
                fix: "This indicates a routing bug in imla-cli; please file an issue."
            )
        }
