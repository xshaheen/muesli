...
final class LocalVQEAudioProcessor: MeetingAecProcessor {
    ...
    init(modelURL: URL, libraryURL: URL, threads: Int) throws {
        ...
        sampleRate = Int(imla_localvqe_sample_rate(context))
        frameSize = Int(imla_localvqe_hop_length(context))
    ...
    deinit {
        if let context {
            imla_localvqe_destroy(context)
        }
    }

    func reset() {
        guard let context else { return }
        imla_localvqe_reset(context)
    }
    ...
    func processFrame(mic: [Float], reference: [Float]) throws -> [Float] {
        ...
        let status = mic.withUnsafeBufferPointer { micBuffer in
            reference.withUnsafeBufferPointer { referenceBuffer in
                output.withUnsafeMutableBufferPointer { outputBuffer in
                    imla_localvqe_process_frame_f32(
                        context,
        ...
        guard status == 0 else {
            throw LocalVQEError.processFailed(String(cString: imla_localvqe_last_error(context)))
        }
        return output
    }
}
