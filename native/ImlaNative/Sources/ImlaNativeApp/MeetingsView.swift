...
struct MeetingsView: View {
        ...
        ScrollView {
            ...
            VStack(alignment: .leading, spacing: ImlaTheme.spacing24) {
                ...
                if presentation.meetings.isEmpty {
                    ...
                    LazyVStack(spacing: ImlaTheme.spacing12) {
        ...
        VStack(alignment: .leading, spacing: ImlaTheme.spacing16) {
            ...
            ForEach(groups) { group in
                HStack(alignment: .top, spacing: 20) {
                    ...
                    VStack(alignment: .center, spacing: 2) {
                        Text(group.dayNumber)
                            ...
                            .foregroundStyle(group.isToday ? ImlaTheme.accent : ImlaTheme.textPrimary)
                        Text(group.dayLabel)
                            .font(ImlaTheme.font(size: 11, weight: .medium))
                            .foregroundStyle(group.isToday ? ImlaTheme.accent : ImlaTheme.textSecondary)
                        ...
                        ForEach(group.events) { event in
                            HStack(spacing: 8) {
                                RoundedRectangle(cornerRadius: 1.5, style: .continuous)
                                    .fill(group.isToday ? ImlaTheme.accent : ImlaTheme.textSecondary.opacity(0.4))
                                    ...
                                    Text("Add to folder")
                                        ...
                                        .overlay(
                                            RoundedRectangle(cornerRadius: 4, style: .continuous)
                                                .strokeBorder(ImlaTheme.surfaceBorder, lineWidth: 0.5)
                                        )
                                }
                ...
                if group.id != lastGroupId {
                    Divider()
                        .foregroundStyle(ImlaTheme.surfaceBorder)
                }
            }
        }
        .padding(20)
    ...
    private func hideEventButton(_ event: UnifiedCalendarEvent) -> some View {
        Button {
            withAnimation(ImlaTheme.Motion.easedOut(0.2)) {
                controller.hideCalendarEvent(event)
            }
        } label: {
            Image(systemName: "xmark")
                .font(.system(size: 9, weight: .medium))
                .foregroundStyle(ImlaTheme.textSecondary.opacity(0.6))
    ...
    private func browserHeader(meetingCount: Int) -> some View {
        VStack(alignment: .leading, spacing: ImlaTheme.spacing8) {
            ViewThatFits(in: .horizontal) {
                HStack(alignment: .top, spacing: ImlaTheme.spacing16) {
                    browserHeaderTitle
    ...
    private var browserHeaderActions: some View {
        HStack(spacing: ImlaTheme.spacing8) {
            Button {
                ...
                .foregroundStyle(ImlaTheme.backgroundBase)
                .padding(.horizontal, ImlaTheme.spacing12)
                .padding(.vertical, 8)
                .background(appState.isMeetingRecording || appState.isMeetingStarting ? ImlaTheme.surfacePrimary : ImlaTheme.accent)
        ...
        HStack(spacing: ImlaTheme.spacing12) {
            ...
            if meeting.status == .recording {
                Button {
                    ...
                    .foregroundStyle(appState.isMeetingRecordingPaused ? ImlaTheme.backgroundBase : ImlaTheme.textPrimary)
                    .padding(.horizontal, ImlaTheme.spacing12)
                    .padding(.vertical, 8)
                    .background(appState.isMeetingRecordingPaused ? ImlaTheme.accent : ImlaTheme.surfacePrimary)
                    .clipShape(RoundedRectangle(cornerRadius: ImlaTheme.cornerSmall, style: .continuous))
                    .overlay(
                        RoundedRectangle(cornerRadius: ImlaTheme.cornerSmall, style: .continuous)
                            .strokeBorder(appState.isMeetingRecordingPaused ? ImlaTheme.accent.opacity(0.35) : ImlaTheme.surfaceBorder, lineWidth: 1)
                    )
                }
                .buttonStyle(.plain)
    ...
    private func activeMeetingStatusColor(for meeting: MeetingListRecord) -> Color {
        guard meeting.status == .recording else { return ImlaTheme.accent }
        return appState.isMeetingRecordingPaused ? ImlaTheme.transcribing : ImlaTheme.recording
    }
    ...
    private var sortButton: some View {
        Menu {
            ...
            .foregroundStyle(selectedSort != .newestFirst ? ImlaTheme.accent : ImlaTheme.textSecondary)
            .padding(.horizontal, 8)
            .padding(.vertical, 3)
            .background(selectedSort != .newestFirst ? ImlaTheme.accent.opacity(0.12) : ImlaTheme.surfacePrimary.opacity(0.5))
            .clipShape(Capsule())
        }
        .menuStyle(.borderlessButton)
        .fixedSize()
    ...
    private var dateFilterButton: some View {
        Menu {
            ...
            .foregroundStyle(selectedFilter != .all ? ImlaTheme.accent : ImlaTheme.textTertiary)
            .padding(.horizontal, selectedFilter != .all ? 8 : 0)
            .padding(.vertical, 3)
            .background(selectedFilter != .all ? ImlaTheme.accent.opacity(0.12) : Color.clear)
            .clipShape(Capsule())
        }
        .menuStyle(.borderlessButton)
        .fixedSize()
    ...
    private var emptyState: some View {
        ...
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(ImlaTheme.backgroundRaised)
        .clipShape(RoundedRectangle(cornerRadius: ImlaTheme.cornerXL, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: ImlaTheme.cornerXL, style: .continuous)
                .strokeBorder(ImlaTheme.surfaceBorder, lineWidth: 1)
        )
    }
