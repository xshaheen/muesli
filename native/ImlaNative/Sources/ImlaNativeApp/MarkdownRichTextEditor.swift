...
private extension NSAttributedString.Key {
    static let imlaMarkdownStrong = NSAttributedString.Key("com.xshaheen.imla.markdown.strong")
    static let imlaMarkdownEmphasis = NSAttributedString.Key("com.xshaheen.imla.markdown.emphasis")
    static let imlaMarkdownCode = NSAttributedString.Key("com.xshaheen.imla.markdown.code")
}
    ...
    final class Coordinator: NSObject, NSTextViewDelegate {
        ...
        private func inlineAttributedString(
            ...
            for run in parsed.runs {
                ...
                if isCode {
                    ...
                    runAttributes[.imlaMarkdownCode] = true
                }
                if isEmphasized {
                    font = italicVersion(of: font)
                    runAttributes[.imlaMarkdownEmphasis] = true
                }
                if isStrong {
                    font = boldVersion(of: font)
                    runAttributes[.imlaMarkdownStrong] = true
                }
        ...
        private func markdownInline(from storage: NSTextStorage, contentRange: NSRange) -> String {
            ...
            storage.enumerateAttributes(in: contentRange) { attrs, range, _ in
                ...
                let isStrong = attrs[.imlaMarkdownStrong] as? Bool == true
                    || (!isHeading && isBold(font))
                let isEmphasized = attrs[.imlaMarkdownEmphasis] as? Bool == true
                let isCode = attrs[.imlaMarkdownCode] as? Bool == true
