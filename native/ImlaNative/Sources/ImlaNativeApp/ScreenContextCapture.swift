...
enum DictationContextCapture {
    ...
    private static func capture(
        ...
        fputs("[imla-native] dictation context: app=\(appName) docContext=\(docContext.count) chars selectedText=\(selectedText.count) chars url=\(url ?? "none")\n", stderr)
...
enum ScreenContextCapture {
    ...
    private static func captureWindow(
        ...
        guard let focusedWindow = DictationContextCapture.focusedWindowSnapshot(
            ...
            fputs("[imla-native] screen context: focused window is not available for \(appName)\n", stderr)
            return nil
        }
        ...
        guard let windowID = focusedWindowID(
            ...
            fputs("[imla-native] screen context: focused window could not be bound for \(appName)\n", stderr)
            return nil
        }
        ...
        guard let image = CGWindowListCreateImage(
            ...
            fputs("[imla-native] screen context: screenshot capture failed\n", stderr)
            return nil
        }

        do {
            let text = try await ocrImage(image)
            fputs("[imla-native] screen context: captured \(text.count) \(logLabel) chars from \(appName)\n", stderr)
            ...
            fputs("[imla-native] screen context: \(logLabel) failed: \(error)\n", stderr)
            return nil
        }
    }
