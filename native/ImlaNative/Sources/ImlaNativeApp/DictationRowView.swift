...
struct DictationRowView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .top, spacing: ImlaTheme.spacing20) {
                Text(timeOnly)
                    .font(ImlaTheme.mono(size: 13, weight: .medium))
                    .foregroundStyle(ImlaTheme.textTertiary)
                    .frame(width: 80, alignment: .leading)
                    .padding(.top, 2)

                VStack(alignment: .leading, spacing: ImlaTheme.spacing8) {
                    HStack(alignment: .firstTextBaseline, spacing: ImlaTheme.spacing8) {
                        if isComputerUseCommand {
                            Text("CUA")
                                .font(ImlaTheme.mono(size: 10, weight: .bold))
                        ...
                        Text(record.rawText)
                            .font(ImlaTheme.font(size: 14, weight: .regular))
                        ...
                        Text(event.body)
                            .font(
                                ["model_output", "quil_model"].contains(event.kind)
                                    ? ImlaTheme.mono(size: 12)
                                    : ImlaTheme.font(size: 12)
                            )
                            .foregroundStyle(ImlaTheme.textPrimary)
...
struct AudioOnlyDictationRowView: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: ImlaTheme.spacing20) {
