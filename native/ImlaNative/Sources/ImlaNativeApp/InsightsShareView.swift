...
struct InsightsShareSheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text("Share your activity")
                        .font(ImlaTheme.font(size: 20, weight: .semibold))
                        .tracking(-0.4)
            ...
            .accessibilityLabel("Preview of your Imla activity image")
            ...
            if let saveErrorMessage {
                ...
                .padding(10)
                .background(ImlaTheme.danger.opacity(0.08))
                .clipShape(RoundedRectangle(cornerRadius: 9, style: .continuous))
                .overlay(RoundedRectangle(cornerRadius: 9, style: .continuous).strokeBorder(ImlaTheme.danger.opacity(0.18)))
            }
    ...
    private func saveImage() {
        ...
        panel.nameFieldStringValue = "Imla activity – \(rangeLabel).png"
    ...
    private func showConfirmation(_ message: String) {
        withAnimation(ImlaTheme.Motion.easedOut(0.16)) { confirmation = message }
        Task { @MainActor in
            try? await Task.sleep(for: .seconds(2))
            guard confirmation == message else { return }
            withAnimation(ImlaTheme.Motion.easedOut(0.16)) { confirmation = nil }
        }
    }
}
...
private struct InsightsShareCard: View {
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack {
                    ...
                    Spacer()
                    Text("github.com/xshaheen/muesli")
                        .font(ImlaTheme.font(size: 15, weight: .semibold))
    ...
    private func shareDatum(value: String, label: String) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            Text(value)
                .font(ImlaTheme.numeric(size: 30, weight: .semibold))
                .tracking(-0.8)
                .monospacedDigit()
                .foregroundStyle(pale)
            Text(label)
                .font(ImlaTheme.font(size: 12, weight: .bold))
                .tracking(1.7)
                .foregroundStyle(muted)
        }
...
private struct ImlaShareMark: View {
    let color: Color
...
enum InsightsBrandAssets {
    ...
    static let appIcon = image(
        bundledName: "imla_app_icon",
        extension: "png",
        repositoryPath: "assets/imla_app_icon.png"
    )
