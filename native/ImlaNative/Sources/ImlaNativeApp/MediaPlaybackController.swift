...
final class MediaPlaybackController: MediaPlaybackManaging {
    ...
    init(
        ...
        queue: DispatchQueue = DispatchQueue(label: "com.xshaheen.imla.media-playback")
    ) {
        self.client = client
        self.queue = queue
    }
    ...
    private func handleRestorePlaybackState(_ playbackState: MediaPlaybackState, token: Int) {
        ...
        // leaving media stranded after Imla paused it.
...
private final class NowPlayingMediaRemoteClient {
    ...
    private let timeoutQueue = DispatchQueue(label: "com.xshaheen.imla.media-playback.now-playing-timeout")
