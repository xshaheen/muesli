...
enum RecordingWaveformCacheFiles {
    ...
    static func removeLegacyJSONWaveformCaches(
        ...
        if removed > 0 {
            logger?("[imla-native] cleaned up \(removed) legacy waveform JSON cache file\(removed == 1 ? "" : "s")")
        }
    ...
    static func sweepOrphanedCachedWaveforms(
        ...
        if removed > 0 {
            logger?("[imla-native] cleaned up \(removed) orphaned waveform cache file\(removed == 1 ? "" : "s")")
        }
        return .completed(removed: removed)
    }
...
struct RecordingArtifactPlayerView: View {
    ...
    var body: some View {
        ...
        .background(ImlaTheme.backgroundRaised.opacity(0.74))
        ...
        GeometryReader { proxy in
            Canvas { context, size in
                ...
                for index in 0..<barCount {
                    ...
                    let color = x <= playedX ? ImlaTheme.accent : ImlaTheme.textTertiary.opacity(0.24)
                    context.fill(
