...
enum ICloudSyncRecoveryPolicy {
    static func action(
        ...
        supportsLegacyReconnect: Bool = ImlaICloudSyncEngine.cloudKitEnvironmentKeyComponent == "production"
...
struct IPhoneBridgeCard: View {
    ...
    var body: some View {
        ...
        .alert("Reconnect iCloud sync?", isPresented: $isReconnectConfirmationPresented) {
            ...
            Text("Imla will reconnect this Mac to the currently signed-in iCloud account and resync eligible text. Local history and audio stay on this Mac.")
        }
        .alert("Reset iCloud sync?", isPresented: $isResetConfirmationPresented) {
            ...
            Text("Imla will turn off sync and clear this Mac's local iCloud sync state. Local history and audio stay on this Mac, and CloudKit data is not deleted. Turn sync on afterward to set up the currently signed-in iCloud account.")
        }
    }
    ...
    private var bridgeIconColor: Color {
        switch bridgeState {
        case .active: return ImlaTheme.success
        case .needsICloud, .needsReconnection, .needsAccountReplacement:
            return ImlaTheme.transcribing
        case .error: return ImlaTheme.danger
        default: return ImlaTheme.accent
        }
    }
    ...
    private var buttonHelp: String {
        ...
        if flowAction == .waitingForDevice {
            return "Waiting for Imla on your other device"
        }
        switch bridgeState {
        case .active:
...
private struct BridgeSyncIcon: View {
    ...
    private func updateRotation(animated: Bool) {
        ...
        withAnimation(ImlaTheme.Motion.repeating(0.9, autoreverses: false)) {
            rotationDegrees = 360
        }
    }
}
