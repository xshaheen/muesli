...
struct MeetingListItemView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: ImlaTheme.spacing8) {
            ...
            HStack(spacing: ImlaTheme.spacing4) {
        ...
        .background(isSelected ? ImlaTheme.surfaceSelected : ImlaTheme.backgroundRaised)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: ImlaTheme.cornerLarge, style: .continuous)
                .strokeBorder(
                    isSelected ? ImlaTheme.accent.opacity(0.35) : ImlaTheme.surfaceBorder,
                    lineWidth: 1
                )
        )
    ...
    private func relationshipIcon(_ systemName: String, help: String) -> some View {
        Image(systemName: systemName)
            ...
            .foregroundStyle(ImlaTheme.accent.opacity(0.9))
        ...
        Button {
            ...
            Image(systemName: record.folderID != nil ? "folder.fill" : "folder.badge.plus")
                ...
                .foregroundStyle(
                    record.folderID != nil
                        ? ImlaTheme.accent
                        : (isHovering ? ImlaTheme.textSecondary : ImlaTheme.textTertiary)
                )
                .frame(width: 24, height: 24)
                .contentShape(Rectangle())
        }
    ...
    private var deleteButton: some View {
        Button {
            ...
            Image(systemName: "trash")
                ...
                .foregroundStyle(
                    isHovering
                        ? ImlaTheme.danger.opacity(0.85)
                        : ImlaTheme.textTertiary
                )
                .frame(width: 24, height: 24)
                .contentShape(Rectangle())
        }
    ...
    private var sourceIndicator: AnyView? {
        ...
        return AnyView(RecordingArtifactAvailabilityBadge(owner: .meeting(record.id)))
    }

    private var isImportedAudio: Bool {
        record.source == .audioImport
    }
    ...
    private func sourceBadge(icon: String, label: String, help: String) -> some View {
        ...
        .foregroundStyle(isImportedAudio ? ImlaTheme.accent : ImlaTheme.textSecondary)
        .padding(.horizontal, 6)
        .padding(.vertical, 2)
        .background((isImportedAudio ? ImlaTheme.accent : ImlaTheme.textSecondary).opacity(0.12))
        .clipShape(Capsule())
        .help(help)
        .accessibilityLabel(help)
    }
