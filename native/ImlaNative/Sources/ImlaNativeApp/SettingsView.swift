...
struct SettingsView: View {
    ...
    private var calendarSourceGroups: [CalendarSourceGroup] {
        ...
        if appState.isGoogleCalendarAuthenticated && !appState.availableGoogleCalendars.isEmpty {
            ...
            groups.append(CalendarSourceGroup(
                ...
                subtitle: "Connected directly to Imla",
    ...
    private var calendarSourcesControl: some View {
        VStack(alignment: .leading, spacing: ImlaTheme.spacing16) {
            Text("Calendar sources are listed first, with their calendars underneath. Disabled calendars are hidden from Imla — no notifications, no Coming Up, no meeting detection.")
                .font(ImlaTheme.caption())
                .foregroundStyle(ImlaTheme.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            if appState.isGoogleCalendarAuthenticated && !appState.availableEventKitCalendars.isEmpty {
                Text("Google calendars may appear once from macOS Calendar and once from Imla's Google connection. Turn off both copies to hide that calendar completely.")
                    .font(ImlaTheme.caption())
                    .foregroundStyle(ImlaTheme.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
