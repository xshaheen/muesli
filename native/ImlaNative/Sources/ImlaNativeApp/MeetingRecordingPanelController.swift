...
private final class MeetingRecordingPanelContentView: NSView {
    ...
    override func mouseUp(with event: NSEvent) {
        owner?.pointerUp(didDrag: didDrag, isDiscardModifier: event.modifierFlags.contains(.option))
...
final class MeetingRecordingPanelController: NSObject {
    ...
    /// Bumped by every `close()` and every `showRecording`. A morph completion that lands
    /// after its recording ended carries a stale generation and is ignored, so it cannot
    /// clear the *next* recording's `isMorphing`, re-pin its size limits and relayout it
    /// mid-transition.
    ...
    var morphGenerationForTesting: Int { morphGeneration }
    ...
    func showRecording(
        ...
        layout = .pill
        appliedContent = nil
        // Retires any morph the previous recording left in flight before this one starts
        // laying itself out.
        morphGeneration += 1
        isMorphing = false
    ...
    func beginFinalizing(ownerID: UUID, status: String = "Finalizing") {
        ...
        // under a pointer that never moved. Only suppress when the pointer really is over
        // the object: a Stop from the status bar, a notification, auto-stop or the 3 h limit
        // folds with the pointer elsewhere, and `pointerExited` — the only clearer — never
        // fires without a prior `mouseEntered`, so an unconditional flag sticks forever.
        hoverSuppressedUntilExit = isPointerInside
    ...
    /// The one exit every mouse-up takes.
    ///
    /// The pointer lifecycle always ends first: `pointerInteractionEnded` is the only writer
    /// of `isPointerDown = false`, and the hover fold refuses to run while a press is down.
    /// Dispatching the discard instead of ending the press left the row unfolded over the
    /// user's call for the rest of the recording. Ending with `didDrag: false` saves nothing.
    func pointerUp(didDrag: Bool, isDiscardModifier: Bool) {
        pointerInteractionEnded(didDrag: didDrag)
        if isDiscardModifier { discardRequested() }
    }
    ...
    private func minimizePanel(userInitiated: Bool) {
        ...
        bodyCoordinator.releaseFocus()
        // After a header-button minimize the pointer is still over the object, so the row
        // waits for it to leave. The status bar's "Minimize Meeting Panel" minimizes from
        // across the screen; suppressing then would arm a flag only `pointerExited` clears,
        // and it never fires for a pointer that was never inside.
        hoverSuppressedUntilExit = isPointerInside
        setLayout(.pill, animated: true)
    ...
    private func pillCustomActions() -> [NSAccessibilityCustomAction] {
        // R9/R17: every one of these selectors no-ops outside recording and paused, so while
        // the pill is finalizing VoiceOver must not offer them at all — an action that is
        // announced and then does nothing reads as a broken control, not a disabled one.
        guard state == .recording || state == .paused else { return [] }
