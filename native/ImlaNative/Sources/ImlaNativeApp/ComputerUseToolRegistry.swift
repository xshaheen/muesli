...
enum ComputerUseToolRegistry {
    static let catalogVersion = "imla-cua-tools-v1"
    ...
    static let definitions: [ComputerUseToolDefinition] = [
        ...
        definition(.moveCursor, "Move the visible Imla CUA cursor to a screenshot coordinate without clicking. Use this before uncertain coordinate clicks to show intent.", required: ["screenshot_id", "x", "y"], properties: [
        ...
        definition(.typeText, "Type literal text using keyboard input. Prefer app_name/app_bundle_id and, when available, element_index/element_id so Imla can activate the app and focus the editable target before typing.", required: ["text"], properties: [
