...
enum DictationCleanupPromptComposer {
    ...
    static func systemPrompt(
        ...
        // is Imla's own and the user cannot grow it.
    ...
    static func systemPrompt(
        ...
        // Imla's own correctness rule, not a preference a mode may replace.
        ...
        if let mode, mode.overrideDefaultInstructions,
           CustomInstructions.normalized(mode.instructions).isEmpty == false {
            ...
            // drop; it is Imla's own untrusted-<APP-CONTEXT> and preserve-meaning
...
actor TranscriptionCoordinator {
    ...
    func preload(
        ...
        do {
            ...
            fputs("[imla-native] preload failed for \(backend.backend)/\(backend.model): \(error)\n", stderr)
        }
    }
    ...
    private func loadBackendModels(
        ...
        case "whisper":
            ...
            fputs("[imla-native] WhisperKit warmup: running silent audio for CoreML compilation...\n", stderr)
            ...
            fputs("[imla-native] WhisperKit warmup complete\n", stderr)
            progress?(1.0, nil)
        ...
        case "nemotron35":
            if #available(macOS 15, *) {
                ...
                fputs("[imla-native] Nemotron 3.5 warmup: running silent chunk for ANE compilation...\n", stderr)
                ...
                fputs("[imla-native] Nemotron 3.5 warmup complete\n", stderr)
            } else {
                throw NSError(domain: "ImlaTranscriptionRuntime", code: 1, userInfo: [
        ...
        case "cohere":
            if #available(macOS 15, *) {
                ...
                throw NSError(domain: "ImlaTranscriptionRuntime", code: 4, userInfo: [
        ...
        case "indicasr":
            if #available(macOS 15, *) {
                ...
                throw NSError(domain: "ImlaTranscriptionRuntime", code: 6, userInfo: [
        ...
        case "gemma4-litert":
            if #available(macOS 15, *) {
                ...
                throw NSError(domain: "ImlaTranscriptionRuntime", code: 7, userInfo: [
        ...
        default:
            throw NSError(domain: "ImlaTranscriptionRuntime", code: 5, userInfo: [
    ...
    func preloadMeetingHelpers(trigger: DiarizerPreloadTrigger = .unspecified) async {
        if vadManager == nil {
            do {
                ...
                fputs("[imla-native] Silero VAD loaded\n", stderr)
            } catch {
    ...
    private func finishDiarizerLoad(
        ...
        case .success(let models):
            ...
            fputs(
                "[imla-native] Speaker diarization loaded (compute: \(policy.computePolicy.rawValue))\n",
                stderr
            )
            outcome = .succeeded
        case .failure(let error):
            ...
            fputs("[imla-native] Diarization load failed (non-critical): \(reportedError)\n", stderr)
            outcome = .failed
        }
    ...
    private func postProcessorSnapshot(
        ...
        return PostProcessorSnapshot(
            ...
            modelURL: runtime.modelURL ?? postProcessorModelURL,
    ...
    func transcribeDictationWithCleanupOutcome(
        ...
        if backend.backend == "cohere", let vadManager {
            do {
                ...
                if !hasSpeech {
                    fputs("[imla-native] VAD: dictation is silent, skipping Cohere transcription\n", stderr)
                ...
                fputs("[imla-native] VAD check failed, transcribing anyway: \(error)\n", stderr)
            }
        }
    ...
    func transcribeMeetingChunkWithEvidence(
        ...
        if let vadManager {
            do {
                ...
                let hasSpeech = vadResults.contains { $0.probability > 0.5 }
                if !hasSpeech {
                    fputs("[imla-native] VAD: chunk is silent, skipping transcription\n", stderr)
    ...
    func diarizeSystemAudio(at url: URL) async throws -> DiarizationResult? {
        guard let diarizerManager, diarizerManager.isAvailable else {
            fputs("[imla-native] diarization not available, skipping\n", stderr)
            return nil
        }
        fputs("[imla-native] running speaker diarization on system audio...\n", stderr)
        ...
        fputs("[imla-native] diarization complete: \(result.segments.count) segments, \(speakerCount) speakers\n", stderr)
        return result
    }
    ...
    private func cleanMeetingTranscript(_ result: SpeechTranscriptionResult) -> SpeechTranscriptionResult {
        ...
        if !result.text.isEmpty, cleaned.text.isEmpty {
            fputs("[imla-native] dropped non-speech chunk artifact: \"\(result.text)\"\n", stderr)
        }
        return cleaned
    }
    ...
    private func routeToBackend(
        ...
        case "gemma4-litert":
            ...
            return try await transcribeWithGemma4LiteRT(
                url: url,
                model: Gemma4LiteRTModel.resolved(backend.model)
            )
        case "apple-speech":
            if #available(macOS 26.0, *) {
                // Apple Speech resolves a bare language code to a supported locale,
                // so a pinned profile language can drive it directly.
                let languageIdentifier: String
                switch languageDecision {
                case .pinned(let selected)?, .fixed(let selected)?:
                    languageIdentifier = selected.rawValue
                case .constrainedCandidates?:
                    throw LanguageRoutingIncompatibility.constrainedCandidatesUnsupported
                case .incompatible?:
                    preconditionFailure("handled before backend routing")
    ...
    private func transcribeWithFluidAudio(url: URL) async throws -> SpeechTranscriptionResult {
        fputs("[imla-native] transcribing with FluidAudio: \(url.lastPathComponent)\n", stderr)
        let result = try await fluidTranscriber.transcribe(wavURL: url)
        fputs("[imla-native] FluidAudio result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
    ...
    private func transcribeWithParakeetUnified(url: URL) async throws -> SpeechTranscriptionResult {
        fputs("[imla-native] transcribing with Parakeet Unified: \(url.lastPathComponent)\n", stderr)
        let result = try await parakeetUnifiedTranscriber.transcribe(wavURL: url)
        fputs("[imla-native] Parakeet Unified result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
        let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
        return SpeechTranscriptionResult(
            text: text,
    ...
    private func transcribeWithWhisperKit(
        ...
        fputs("[imla-native] transcribing with WhisperKit: \(url.lastPathComponent)\n", stderr)
        let result = try await whisperTranscriber.transcribe(
            wavURL: url,
            vocabulary: vocabulary,
            language: language
        )
        fputs("[imla-native] WhisperKit result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
        let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
        return SpeechTranscriptionResult(
            text: text,
            segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
    ...
    private func transcribeWithSenseVoice(url: URL) async throws -> SpeechTranscriptionResult {
        fputs("[imla-native] transcribing with SenseVoice: \(url.lastPathComponent)\n", stderr)
        let result = try await senseVoiceTranscriber.transcribe(wavURL: url)
        fputs("[imla-native] SenseVoice result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
        let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
        return SpeechTranscriptionResult(
            text: text,
    ...
    private func transcribeWithCohere(
        ...
        if #available(macOS 15, *) {
            fputs("[imla-native] transcribing with Cohere Transcribe: \(url.lastPathComponent)\n", stderr)
            let result = try await cohereTranscriber.transcribe(wavURL: url, language: language)
            fputs("[imla-native] Cohere Transcribe result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
            ...
            throw NSError(domain: "Imla", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Cohere Transcribe requires macOS 15 or later.",
            ])
        }
    }
    ...
    private func transcribeWithNemotron35(
        ...
        if #available(macOS 15, *) {
            fputs("[imla-native] transcribing with Nemotron 3.5: \(url.lastPathComponent)\n", stderr)
            ...
            fputs("[imla-native] Nemotron 3.5 result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
