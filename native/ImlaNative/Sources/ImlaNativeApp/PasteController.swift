...
enum PasteController {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
            ...
            if requireStagedClipboardOwnership {
                guard didStageText else {
                    // Restore only when Imla still owns the cleared pasteboard. If another
            ...
            if didDispatchPaste {
                // Report whether Imla still owned the text it staged. The paste is
    ...
    static func typeText(_ text: String) {
        ...
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            fputs("[imla-native] failed to create event source for typeText\n", stderr)
            return
        }
        for char in text {
    ...
    private static func simulateCopy() -> Bool {
        guard let source = CGEventSource(stateID: .combinedSessionState) else {
            fputs("[imla-native] failed to create event source for copy\n", stderr)
            return false
        }
        ...
        guard let commandDown = CGEvent(keyboardEventSource: source, virtualKey: keyCode, keyDown: true),
            ...
            fputs("[imla-native] failed to create keyboard events for copy\n", stderr)
            return false
        }
    ...
    private static func simulatePaste() -> Bool {
        ...
        guard let commandDown = CGEvent(keyboardEventSource: source, virtualKey: keyCode, keyDown: true),
            ...
            fputs("[imla-native] failed to create keyboard events for paste\n", stderr)
            return false
        }
        ImlaSyntheticKeyboardEvent.mark(commandDown)
        ImlaSyntheticKeyboardEvent.mark(commandUp)
    ...
    private static func postPhysicalKey(source: CGEventSource, keyCode: CGKeyCode, flags: CGEventFlags) {
        ...
        ImlaSyntheticKeyboardEvent.mark(keyDown)
        ImlaSyntheticKeyboardEvent.mark(keyUp)
        keyDown.flags = flags
        keyUp.flags = flags
