...
final class DictationTextContextMonitor {
    private static let logger = Logger(subsystem: "com.xshaheen.imla", category: "TextContext")

    var onSample: ((DictationTextContextSample?) -> Void)?
    var onActivityChanged: ((DictationFollowerActivity) -> Void)?
    var onFocusChanged: (() -> Void)?
    var onEscape: (() -> Void)?
    /// Sampling stopped outright (monitor stopped, or the frontmost app is Imla itself,
