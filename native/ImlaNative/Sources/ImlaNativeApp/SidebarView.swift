...
struct SidebarView: View {
    ...
    private var collapsedSidebar: some View {
        VStack(spacing: ImlaTheme.spacing4) {
            sidebarToggleButton
                .padding(.top, Self.titlebarHeight + ImlaTheme.spacing8)
    ...
    private func collapsedItem(tab: DashboardTab, icon: String, label: String) -> some View {
        Button {
            ...
            Image(systemName: icon)
                .font(.system(size: 16, weight: .medium))
                .foregroundStyle(appState.selectedTab == tab ? ImlaTheme.accent : ImlaTheme.textSecondary)
                .frame(width: collapsedItemSize, height: collapsedItemSize)
                .background(appState.selectedTab == tab ? ImlaTheme.surfacePrimary : Color.clear)
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: ImlaTheme.spacing4) {
                HStack(spacing: ImlaTheme.spacing12) {
                    Group {
                        if appState.config.menuBarIcon == "imla",
            ...
            HStack(spacing: ImlaTheme.spacing12) {
                ...
                Button {
                    ...
                    Image(systemName: meetingsExpanded ? "chevron.down" : "chevron.right")
                        ...
                        .foregroundStyle(isSelected ? ImlaTheme.textSecondary : ImlaTheme.textTertiary)
    ...
    private var modelPreparationStatus: some View {
        if let title = appState.modelPreparationTitle {
            ...
            .padding(.horizontal, sidebarRowOuterPadding)
            .padding(.bottom, ImlaTheme.spacing4)
        }
    }
    ...
    private func meetingFilterRow(
        ...
        .background(
            RoundedRectangle(cornerRadius: ImlaTheme.cornerSmall, style: .continuous)
                .fill(isSelected ? ImlaTheme.surfaceSelected.opacity(0.6) : Color.clear)
        )
        .contentShape(Rectangle())
    ...
    private func folderRenameField(folder: MeetingFolder) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: ImlaTheme.cornerSmall, style: .continuous)
                .fill(ImlaTheme.surfaceSelected.opacity(0.6))
        )
    }
    ...
    private func toggleFolderCollapse(_ folderID: Int64) {
        withAnimation(ImlaTheme.Motion.eased(0.12)) {
