...
struct MeetingChatView: View {
    ...
    var body: some View {
        ...
        .background {
            ...
            if !isCompact { ImlaTheme.backgroundBase }
        }
    }
    ...
    private var emptyState: some View {
        Text(hasTranscript
            ...
            .font(ImlaTheme.font(size: isCompact ? 11 : 12))
    ...
    private func turnBubbleFill(for role: MeetingChatTurn.Role) -> Color {
        ...
        return role == .user ? ImlaTheme.surfaceSelected : ImlaTheme.backgroundRaised
    }
    ...
    private func copyButton(for text: String) -> some View {
        Button {
            ...
            .font(ImlaTheme.font(size: isCompact ? 10 : 11))
            .foregroundStyle(ImlaTheme.textTertiary)
        }
        .buttonStyle(.plain)
        .padding(.top, 2)
    ...
    private var thinkingRow: some View {
        HStack(spacing: 6) {
            ProgressView()
                .controlSize(.small)
                .tint(usesNeutralGlassStyle ? Color.white.opacity(0.70) : ImlaTheme.accent)
            Text("Thinking…")
                .font(ImlaTheme.font(size: isCompact ? 11 : 12))
                .foregroundStyle(ImlaTheme.textTertiary)
        }
            ...
            HStack(spacing: 6) {
                ForEach(MeetingChatRecipes.visible()) { recipe in
                    Button { run(recipe) } label: {
                        Text(recipe.name)
                            .font(ImlaTheme.font(size: isCompact ? 10 : 11, weight: .medium))
                            .foregroundStyle(ImlaTheme.textSecondary)
                            .lineLimit(1)
                            .fixedSize()
                            .padding(.horizontal, 10)
    ...
    private var inputRow: some View {
        ...
        return HStack(spacing: 8) {
            TextField("Ask anything", text: $draft, axis: .vertical)
                ...
                .font(ImlaTheme.font(size: isCompact ? 12 : 13))
                .lineLimit(1 ... 4)
                ...
                Image(systemName: "arrow.up.circle.fill")
                    ...
                    .foregroundStyle(
                        usesNeutralGlassStyle
                            ...
                            : (canSend ? ImlaTheme.accent : ImlaTheme.textTertiary)
                    )
            }
            .buttonStyle(.plain)
            .disabled(!canSend)
