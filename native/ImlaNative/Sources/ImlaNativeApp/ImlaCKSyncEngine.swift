...
protocol ImlaCKSyncPendingState: AnyObject, Sendable {
...
extension CKSyncEngine.State: ImlaCKSyncPendingState {}

struct ImlaCKSyncIntent: OptionSet, Sendable {
    let rawValue: UInt8
...
struct ImlaCKSyncRequest: Sendable, Equatable {
    let intent: ImlaCKSyncIntent
    let userInitiated: Bool
}

struct ImlaCKSyncRequestQueue: Sendable {
    private(set) var intent: ImlaCKSyncIntent = []
    private(set) var isUserInitiated = false

    var isEmpty: Bool { intent.isEmpty }

    mutating func enqueue(intent: ImlaCKSyncIntent, userInitiated: Bool) {
        self.intent.formUnion(intent)
        isUserInitiated = isUserInitiated || userInitiated
    }

    mutating func consume() -> ImlaCKSyncRequest? {
        guard !intent.isEmpty else { return nil }
        let request = ImlaCKSyncRequest(
            intent: intent,
...
struct ImlaCKSyncPreparationState: Sendable {
...
struct ImlaCKSyncFailedRecordSave: Sendable {
    let record: CKRecord
    let error: CKError
}

struct ImlaCKSyncRecordBatch: Sendable {
...
/// Owns the single CKSyncEngine instance for Imla's private text-record zone.
///
...
actor ImlaCKSyncEngine: CKSyncEngineDelegate {
    static var stateKey: String {
        "cksyncengine.private.ImlaSyncZone.\(ImlaICloudSyncEngine.cloudKitEnvironmentKeyComponent).v1"
    }
    static var accountScopeKey: String {
        "cksyncengine.private.ImlaSyncZone.\(ImlaICloudSyncEngine.cloudKitEnvironmentKeyComponent).account-owner.v1"
    }
    static var productionLegacyScopeMigration: ImlaCKSyncLegacyScopeMigration? {
        guard ImlaICloudSyncEngine.cloudKitEnvironmentKeyComponent == "production" else {
            return nil
        }
        return ImlaCKSyncLegacyScopeMigration(
            accountScopeKey: "cksyncengine.private.ImlaSyncZone.unspecified.account-owner.v1",
            stateKey: "cksyncengine.private.ImlaSyncZone.unspecified.v1"
        )
    }
    ...
    private let legacyScopeMigration: ImlaCKSyncLegacyScopeMigration?
    private var container: CKContainer?
    private var preflight: ImlaICloudSyncEngine?
    private var engine: CKSyncEngine?
    private var preparationState = ImlaCKSyncPreparationState()
    ...
    private func runWithZoneRecovery(
        ...
        do {
            ...
            if ImlaICloudSyncEngine.isICloudAccountContextError(error) {
                await invalidatePreparation(cancelEngine: true)
                throw error
            }
            guard ImlaICloudSyncEngine.isSyncZoneRecoveryError(error) else {
                throw error
            }

            // The private zone can be deleted while Imla is running. Recreate it
    ...
    private func preparePreflightIfNeeded() async throws {
        ...
        if let existingTask = preparationTask,
           let existingTaskID = preparationTaskID {
            ...
            let preflight: ImlaICloudSyncEngine
            if let existing = self.preflight {
                preflight = existing
            } else {
                let created = ImlaICloudSyncEngine(container: resolvedContainer())
    ...
    private func performPreparation(preflight: ImlaICloudSyncEngine) async throws -> Bool {
    ...
    private func resolvedContainer() -> CKContainer {
        ...
        let created = CKContainer(identifier: ImlaICloudSyncEngine.Schema.containerIdentifier)
        container = created
        return created
    }

    func registerNextDirtyBatch(state: any ImlaCKSyncPendingState) throws -> Int {
    ...
    func makeRecordBatch(
        ...
        let relevantChanges: [(CKSyncEngine.PendingRecordZoneChange, CKRecord.ID)] =
            pendingChanges.compactMap { change in
                guard case .saveRecord(let recordID) = change,
                      recordID.zoneID == ImlaICloudSyncEngine.Schema.syncZoneID else {
                    return nil
                }
        ...
        do {
            ...
            fputs(
                "[imla-native] CKSyncEngine local batch read failed: \(String(describing: type(of: error)))\n",
                stderr
            )
            return ImlaCKSyncRecordBatch(recordsToSave: [], staleChanges: [])
        }
        ...
        for (change, recordID) in relevantChanges {
            ...
            let cloudRecord = ImlaICloudSyncEngine.syncZoneCloudRecord(
                from: localRecord,
                baseRecord: conflictBaseRecords[recordID]
            )
            recordsToSave.append(cloudRecord)
        }
        return ImlaCKSyncRecordBatch(
        ...
        do {
            ...
            case .sentRecordZoneChanges(let changes):
                ...
                try handleSentRecordChanges(
                    ...
                    failedRecordSaves: changes.failedRecordSaves.map {
                        ImlaCKSyncFailedRecordSave(record: $0.record, error: $0.error)
                    },
                    state: syncEngine.state
                )
            ...
            case .fetchedDatabaseChanges(let changes):
                if changes.deletions.contains(where: {
                    $0.zoneID == ImlaICloudSyncEngine.Schema.syncZoneID
                }) {
            ...
            fputs("[imla-native] CKSyncEngine event failed: \(String(describing: type(of: error)))\n", stderr)
        }
    }

    func handleFetchedRecords(
        _ cloudRecords: [CKRecord],
        state: any ImlaCKSyncPendingState
    ) throws {
        let records = cloudRecords
            .filter {
                $0.recordID.zoneID == ImlaICloudSyncEngine.Schema.syncZoneID
                    && $0.recordType == ImlaICloudSyncEngine.Schema.textRecordType
            }
            .compactMap(ImlaICloudSyncEngine.syncTextRecord(from:))
    ...
    func handleSentRecordChanges(
        ...
        failedRecordSaves: [ImlaCKSyncFailedRecordSave],
        state: any ImlaCKSyncPendingState
    ) throws {
        for savedRecord in savedRecords {
            guard let syncRecord = ImlaICloudSyncEngine.syncTextRecord(from: savedRecord) else { continue }
            if try store.markTextRecordSynced(
                kind: syncRecord.kind,
                recordName: syncRecord.id,
                changeTag: savedRecord.recordChangeTag,
                systemFields: ImlaICloudSyncEngine.encodedSystemFields(for: savedRecord),
        ...
        for failure in failedRecordSaves {
            ...
            guard recordID.zoneID == ImlaICloudSyncEngine.Schema.syncZoneID else { continue }
            let pending = CKSyncEngine.PendingRecordZoneChange.saveRecord(recordID)

            if failure.error.code == .serverRecordChanged,
               let serverRecord = failure.error.serverRecord,
               let remote = ImlaICloudSyncEngine.syncTextRecord(from: serverRecord),
               let local = try store.textRecordForSync(recordName: recordID.recordName) {
                ...
                // stale, so last-write-wins depends only on Imla's updatedAt field.
    ...
    private func authorizeAccount(
        ...
        if let legacyScopeMigration,
           try store.migrateCloudSyncAccountScope(
            ...
            accountBoundaryBlocked = false
            fputs("[imla-native] CKSyncEngine repaired legacy environment scope\n", stderr)
            return true
        }
        ...
        if !legacyRecordNames.isEmpty {
            ...
            if let legacyAccountRecordVerifier {
                ...
                let resolved = self.preflight
                    ?? ImlaICloudSyncEngine(container: resolvedContainer())
            ...
            guard Self.hasExactAccountProvenance(
                ...
                fputs("[imla-native] CKSyncEngine account provenance unverified\n", stderr)
                return false
            }
        }
