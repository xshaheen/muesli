...
final class StatusBarController: NSObject, NSMenuDelegate {
    ...
    private func rebuildMenu() {
        ...
        menu.addItem(actionItem(title: "Open \(AppIdentity.displayName)", action: #selector(ImlaController.openHistoryWindow as (ImlaController) -> () -> Void)))
        if controller.isMeetingRecording() {
            ...
            menu.addItem(actionItem(title: pauseTitle, action: #selector(ImlaController.toggleMeetingRecordingPause)))
        ...
        if recentRows.isEmpty {
            ...
            for row in recentRows {
                let item = NSMenuItem(title: controller.truncate(row.rawText, limit: 54), action: #selector(ImlaController.copyRecentDictation(_:)), keyEquivalent: "")
        ...
        for option in MeetingSummaryBackendOption.all {
            ...
            let item = NSMenuItem(
                title: "\(prefix)\(option.label)",
                action: #selector(ImlaController.selectMeetingSummaryBackendFromMenu(_:)),
                keyEquivalent: ""
            )
            item.target = controller
            item.representedObject = option.label
    ...
    private func addUpcomingEventGroup(
        ...
        for event in displayedEvents {
            ...
            let item = NSMenuItem(
                title: "\(event.title)\n\(timeStr)",
                action: #selector(ImlaController.startMeetingFromCalendarMenuItem(_:)),
                keyEquivalent: ""
            )
            item.target = controller
    ...
    private func checkForUpdatesItem() -> NSMenuItem {
        let item = NSMenuItem(
            ...
            action: #selector(ImlaController.checkForUpdates),
            keyEquivalent: ""
        )
        item.target = controller
