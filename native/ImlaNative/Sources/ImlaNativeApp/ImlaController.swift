...
enum ImlaBridgeDeviceRefreshPolicy {
...
enum ImlaBridgeCompanionDiscoveryPolicy {
...
private final class DictationLatencyLogWriter: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.xshaheen.imla.dictation-latency-log")
    private let url: URL
...
@MainActor
public final class ImlaController: NSObject {
    ...
    /// in the separate ImlaNativeAppShell executable module, not this library.
    public static weak var current: ImlaController?
    ...
    private static let dictionarySuggestionLogger = Logger(subsystem: "com.xshaheen.imla", category: "DictionarySuggestion")
    private static let meetingCleanupLogger = Logger(subsystem: "com.xshaheen.imla", category: "meeting-cleanup")
    ...
    private var ckSyncEngine: ImlaCKSyncEngine?
    ...
    private var pendingICloudSyncRequests = ImlaCKSyncRequestQueue()
    ...
    init(
        ...
        let resolvedDictationStore = dictationStore ?? DictationStore(
            databaseURL: ImlaPaths.defaultDatabaseURL(appName: AppIdentity.supportDirectoryName)
        )
        ...
        do {
            ...
            fputs("[imla-native] startup migration failed: \(error)\n", stderr)
        ...
        ImlaTheme.accentOverrideHex = loadedConfig.accentOverrideHex
    ...
    func start() {
        ...
        cleanupTemporaryDirectory(
            named: "imla-system-audio",
            logDescription: "leftover temp audio files"
        )
        cleanupTemporaryDirectory(
            named: "imla-meeting-recordings",
        ...
        dataDidChangeObserver = DistributedNotificationCenter.default().addObserver(
            forName: ImlaNotifications.dataDidChange,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor [weak self] in
        ...
        if config.iCloudSyncEnabled {
            if ImlaICloudSyncEngine.hasRequiredEntitlement {
    ...
    private func refreshICloudBridgeDeviceState() {
        appState.iCloudBridgeRemoteDeviceName = ImlaBridgeDeviceIdentity.remoteDeviceDisplayName
        appState.iCloudBridgeRemoteDevicePlatform = ImlaBridgeDeviceIdentity.remoteDevicePlatform
    }
    ...
    func recoverStaleLiveMeetings() {
        ...
        do {
            ...
            fputs("[imla-native] failed to load stale live meetings: \(error)\n", stderr)
            return
        }
        ...
        for meeting in meetings {
            do {
                ...
                fputs("[imla-native] failed to recover stale meeting \(meeting.id): \(error)\n", stderr)
            }
        }
    ...
    private func normalizeMeetingTranscriptionSelectionForAvailability(
        ...
        if config.meetingTranscriptionBackend != resolved.backend ||
            ...
            fputs("[imla-native] meeting transcription model unavailable; switched to \(resolved.label)\n", stderr)
        }
    ...
    func openContributionMilestoneAction(_ action: ContributionMilestoneAction) {
        guard let prompt = appState.contributionMilestonePrompt else { return }
        if action == .tweetAboutImla || action == .postOnLinkedIn {
    ...
    func beginIPhoneBridgeDeviceDiscovery() {
        guard ImlaICloudSyncEngine.hasRequiredEntitlement else {
            disableICloudSyncForUnavailableEntitlement()
            return
        }
        ...
        bridgeCompanionDiscoveryActivity = ProcessInfo.processInfo.beginActivity(
            ...
            reason: "Waiting for an iPhone or iPad to finish Imla sync setup"
        )
        ...
        bridgeCompanionDiscoveryTask = Task { [weak self] in
            ...
            let deadline = clock.now.advanced(by: ImlaBridgeCompanionDiscoveryPolicy.timeout)
            while clock.now < deadline {
                ...
                do {
                    let nextRefresh = min(
                        clock.now.advanced(by: ImlaBridgeCompanionDiscoveryPolicy.retryInterval),
                        deadline
                    )
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                await MainActor.run {
                    ...
                    ImlaBridgeDeviceIdentity.clearRemoteDevice()
    ...
    func handleICloudRemoteNotification(userInfo: [AnyHashable: Any]) {
        guard config.iCloudSyncEnabled,
              (ImlaICloudSyncEngine.isTextRecordSubscriptionNotification(userInfo)
                  || ImlaCKSyncEngine.isSyncNotification(userInfo)) else {
            return
        }
    ...
    private func ensureICloudSubscription() {
        ...
        iCloudSubscriptionTask = Task { [weak self] in
            do {
                ...
                fputs(
                    "[imla-native] failed to prepare CKSyncEngine: \(String(describing: type(of: error)))\n",
                    stderr
                )
                await MainActor.run {
                    guard let self,
    ...
    private func startICloudSync() {
        ...
        let hasKnownCompanionDeviceAtStart = ImlaBridgeDeviceIdentity.hasCompanionRemoteDevice()
        iCloudSyncTask = Task { [weak self] in
            do {
                let forceBridgeDeviceRefresh = ImlaBridgeDeviceRefreshPolicy.shouldForceRefresh(
                ...
                do {
                    ...
                    fputs(
                        "[imla-native] failed to purge old iCloud tombstones: \(String(describing: type(of: error)))\n",
                        stderr
                    )
                }
                await MainActor.run {
                    ...
                    self.refreshICloudBridgeDeviceState()
                    let remoteDeviceName = ImlaBridgeDeviceIdentity.remoteDeviceDisplayName ?? "iPhone"
    ...
    private func presentICloudSyncFailure(_ error: Error, statusPrefix: String) {
        ...
        if let syncError = error as? ImlaCKSyncError {
            switch syncError {
        ...
        } else if ImlaICloudSyncEngine.isICloudAccountAvailabilityError(error) {
    ...
    private func iCloudSyncFailureReason(_ error: Error) -> String {
        ...
        if ImlaICloudSyncEngine.isICloudAccountAvailabilityError(error) {
    ...
    private func resolvedCKSyncEngine() -> ImlaCKSyncEngine {
        if let ckSyncEngine { return ckSyncEngine }
        let lifecycleID = UUID()
        ckSyncEngineLifecycleID = lifecycleID
        let created = ImlaCKSyncEngine(
    ...
    func selectIndicASRLanguage(_ language: BodhanLanguage) {
        updateConfig {
    ...
    func signInWithChatGPT(selectMeetingSummaryBackend shouldSelectMeetingSummaryBackend: Bool = true) async -> String? {
        do {
            ...
            fputs("[imla-native] ChatGPT sign-in failed: \(error)\n", stderr)
    ...
    func signInWithOpenRouter(
        ...
        do {
            ...
            fputs("[imla-native] OpenRouter sign-in failed: \(error.localizedDescription)\n", stderr)
            return error.localizedDescription
        }
    }
    ...
    func startCalendarMonitoring() {
        ...
        // Run one initial reconciliation so changes made while Imla was not
    ...
    /// onboarding where the focused field belongs to Imla's own flow, and it stays away
    ...
    func continueModelPreparationAfterOnboarding(
        ...
        onboardingModelPreparationTask = Task { [weak self] in
            ...
            do {
                ...
                fputs("[imla-native] post-onboarding model preparation failed: \(error)\n", stderr)
            }
        }
    }
    ...
    func relaunchApp() {
        ...
        DispatchQueue.main.async {
            ...
            do {
                ...
                fputs("[imla-native] relaunch failed: \(error)\n", stderr)
            }
    ...
    func downloadModelForOnboarding(
        ...
        guard backend.isDownloaded else {
            throw NSError(
                domain: "ImlaOnboardingModelDownload",
                code: 1,
    ...
    private func resummarize(
        ...
        Task { [weak self] in
            ...
            do {
                ...
                fputs("[imla-native] failed to generate or persist meeting summary: \(error)\n", stderr)
                await MainActor.run {
    ...
    func updateMeetingTitle(id: Int64, title: String) {
        ...
        do {
            ...
            fputs("[imla-native] failed to update meeting title \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    func updateMeetingTranscript(id: Int64, transcript: String) {
        do {
            ...
            fputs("[imla-native] failed to update meeting transcript \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    func updateMeetingManualNotes(id: Int64, notes: String) {
        ...
        do {
            ...
            fputs("[imla-native] failed to update manual notes for \(id): \(error)\n", stderr)
        }
        syncAppState()
    }
    ...
    private func persistCachedMeetingManualNotes(id: Int64, notes: String, sync: Bool) {
        ...
        do {
            ...
            fputs("[imla-native] failed to persist manual notes for \(id): \(error)\n", stderr)
        }
        if sync {
            syncAppState()
        }
    ...
    private func flushCachedMeetingTitle(id: Int64) {
        ...
        do {
            ...
            fputs("[imla-native] failed to flush cached meeting title \(id): \(error)\n", stderr)
        }
    }
    ...
    func createMeetingFromCalendarEvent(_ event: UnifiedCalendarEvent, folderID: Int64?) {
        ...
        if let existing = try? dictationStore.meetingByCalendarOccurrence(occurrence) {
            ...
            syncAppState()
            fputs("[imla-native] calendar event already exists as meeting \(existing.id), moved to folder\n", stderr)
            return
        }
        ...
        do {
            ...
            scheduleICloudSyncAfterLocalChange()
            syncAppState()
            fputs("[imla-native] created meeting from calendar event: \(event.title) (folder=\(folderID.map(String.init) ?? "none"))\n", stderr)
        } catch {
            fputs("[imla-native] failed to create meeting from calendar event: \(error)\n", stderr)
        }
    }
    ...
    func shouldTerminateApplication() -> Bool {
        ...
        case .recording:
            ...
            informativeText = "Quitting now will stop the meeting recording and the current transcript may be lost. Stop the recording first if you want Imla to save notes."
        case .processing:
        ...
        alert.addButton(withTitle: "Keep Imla Running")
    ...
    func startMeetingRecording(
        ...
        do {
            ...
            fputs("[imla-native] failed to create live meeting: \(error)\n", stderr)
        ...
        runMeetingStart(meetingID: meetingID) { [weak self] attemptOwner in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    fputs("[imla-native] failed to start meeting: \(error)\n", stderr)
    ...
    func meetingThreadContext(for meetingID: Int64) -> MeetingThreadContext? {
        do {
            ...
            fputs("[imla-native] failed to resolve meeting thread for \(meetingID): \(error)\n", stderr)
            return nil
        }
    }
    ...
    func resumeFinishedMeeting(meetingID: Int64) {
        ...
        do {
            ...
            fputs("[imla-native] failed to prepare meeting resume \(meetingID): \(error)\n", stderr)
        ...
        runMeetingStart(meetingID: meetingID) { [weak self] attemptOwner in
            ...
            do {
                ...
                if self.meetingStartMeetingID == meetingID {
                    ...
                    fputs("[imla-native] failed to resume meeting: \(error)\n", stderr)
                    self.disarmMeetingAutoStopAfterFailedStart()
                    self.resolveLiveMeetingAfterStartFailure(id: meetingID)
                    self.cancelMeetingRecordingHotkeyToggleAfterFailedStart(meetingID: meetingID)
                    self.meetingMonitor.resumeAfterCooldown()
                ...
                meetingSession.onChunkTranscribed = { [weak self, weak meetingSession] segments, speaker in
                    Task { @MainActor [weak self, weak meetingSession] in
                        ...
                        do {
                            ...
                            fputs("[imla-native] failed to checkpoint live transcript for meeting \(meetingID): \(error)\n", stderr)
                        }
    ...
    private func restoreResumedMeetingIfNeeded(id: Int64) -> Bool {
        ...
        do {
            ...
            fputs("[imla-native] failed to restore resumed meeting \(id): \(error)\n", stderr)
    ...
    private func deleteMeetingDraftAndScheduleSync(id: Int64) {
        do {
            ...
            fputs("[imla-native] failed to delete meeting draft \(id): \(error)\n", stderr)
        }
    }
    ...
    private func updateMeetingStatusAndScheduleSync(id: Int64, status: MeetingStatus) {
        do {
            ...
            fputs("[imla-native] failed to update meeting \(id) status to \(status.rawValue): \(error)\n", stderr)
        }
    }
    ...
    func openDiagnosticIncidentIssue(_ incident: DiagnosticIncident) {
        ...
        DispatchQueue.main.async {
            ...
            NSWorkspace.shared.open([url], withApplicationAt: applicationURL, configuration: configuration) { _, error in
                if let error {
                    fputs("[imla-native] failed to open diagnostic issue URL with activation: \(error)\n", stderr)
                    NSWorkspace.shared.open(url)
                }
            }
        }
    ...
    private func updateActiveMeetingSystemAudioFailure(meetingID: Int64) {
        activeMeetingAudioWarningState.recordSystemAudioFailure(
            message: "System audio was interrupted. Imla is retrying automatically; other participants may be missing until capture resumes."
        )
        let nextWarning = activeMeetingAudioWarningState.resolvedWarning(meetingID: meetingID)
        guard activeMeetingAudioWarning != nextWarning else { return }
        activeMeetingAudioWarning = nextWarning
    ...
    func stopMeetingRecording() {
        ...
        let finalizationTask = Task { [weak self] in
            ...
            do {
                ...
                fputs("[imla-native] meeting transcription failed: \(error)\n", stderr)
    ...
    func rollbackProvisionalCompletedMeeting(
        ...
        do {
            ...
            fputs("[imla-native] failed to roll back provisional meeting \(meetingID): \(error)\n", stderr)
            return false
        }
    ...
    private func finalizeCompletedMeetingRecoveryMetadataBestEffort(meetingID: Int64) {
        do {
            ...
            fputs(
                "[imla-native] completed meeting \(meetingID) retained retryable recovery metadata: \(error)\n",
                stderr
            )
        }
    }
    ...
    private func mergedResumeResult(
        ...
        do {
            ...
            fputs("[imla-native] resume summary regeneration failed: \(error.localizedDescription)\n", stderr)
    ...
    private func cleanupTemporaryDirectory(named directoryName: String, logDescription: String) {
        ...
        if !files.isEmpty {
            fputs("[imla-native] cleaned up \(files.count) \(logDescription)\n", stderr)
        }
    }
            ...
            do {
                ...
                if directAudio {
                    ...
                    await MainActor.run {
                        ...
                        self.dictationMiniIndicator.showToast("Applying audio instruction")
                    }
                } else {
    ...
    private func handleComputerUseStart() {
        ...
        guard ensureComputerUseScreenRecordingAccess(isGranted: CGPreflightScreenCaptureAccess()) else {
            ...
            alert.informativeText = "Imla needs Screen Recording permission to see the apps you ask it to use. Enable it in System Settings, then try your command again."
    ...
    private func shouldRejectDictationForComputerUseActivity() -> Bool {
        ...
        fputs("[imla-native] ignoring dictation start while computer use owns interactive audio\n", stderr)
    ...
    private func shouldIgnoreDictationCleanupForComputerUseActivity() -> Bool {
        ...
        fputs("[imla-native] ignoring dictation cleanup while computer use owns interactive audio\n", stderr)
        hotkeyMonitor.cancelToggleMode()
        return true
    }
    ...
    private func handleDictationAudioSessionEvent(_ event: DictationAudioSessionEvent) {
        ...
        case .streamActive(let sessionID, let capturedAt):
            guard !cancelledDictationAudioSessionIDs.contains(sessionID) else {
                fputs("[imla-native] ignoring stream-active for cancelled session\n", stderr)
                break
            }
        ...
        case .stopped(let eventSessionID, let wavURL):
            ...
            guard let eventSessionID,
                ...
                fputs("[imla-native] ignoring stale stopped event\n", stderr)
                if let wavURL {
                    try? FileManager.default.removeItem(at: wavURL)
                }
                break
        ...
        case .failed(let sessionID, let error, let terminalCapture):
            fputs("[imla-native] recorder start failed: \(error)\n", stderr)
            // A hosted provider may already be streaming this session's audio;
            // the local recorder failing ends that stream too.
            cancelHostedDictation()
            if let sessionID {
                cancelledDictationAudioSessionIDs.remove(sessionID)
            }
    ...
    /// The external app that owned focus, ignoring Imla itself, for dictation
    /// attribution. `lastExternalApp` covers the case where Imla's own window
    ...
    private func handleStart() {
        ...
        fputs("[imla-native] recording start\n", stderr)
        meetingMonitor.suppressWhileActive()
    ...
    private func startNemotronStreamingAsync(
        ...
        Task {
            ...
            fputs("[imla-native] got Nemotron 3.5 transcriber\n", stderr)
            ...
            await MainActor.run {
                guard self.isNemotron35Streaming, self.nemotron35StreamingSessionID == sessionID else {
                    fputs("[imla-native] Nemotron session cancelled before transcriber ready\n", stderr)
                    return
                }
                ...
                controller.onPartialText = { [weak self] fullText in
                    ...
                    DispatchQueue.main.async {
                        ...
                        fputs("[imla-native] streaming partial: +\"\(delta)\" (total \(fullText.count) chars)\n", stderr)
                        if !delta.isEmpty {
                ...
                fputs("[imla-native] Nemotron streaming controller started\n", stderr)
            }
        }
    }
    ...
    private func handleNemotronStreamingStartFailure(
        ...
        let startedAt = dictationStartedAt ?? Date()
        fputs("[imla-native] Nemotron streaming controller failed to start\n", stderr)
        if !isDictationTestMode {
            recordDiagnosticIncident(
    ...
    private func handleNemotronStreamingRuntimeFailure(
        ...
        guard isNemotron35Streaming, nemotron35StreamingSessionID == sessionID else { return }
        let startedAt = dictationStartedAt ?? Date()
        fputs("[imla-native] Nemotron streaming failed: \(error)\n", stderr)
        if !isDictationTestMode {
            recordDiagnosticIncident(
    ...
    private func handleCancel(retainCapture: Bool = true) {
        ...
        fputs("[imla-native] \(retainCapture ? "cancel" : "tap-discard")\n", stderr)
        // Cancelling an in-flight transcription is a real cancel even when the
        // start was blocked, so it must not be swallowed with the blocked-start
        // cleanup.
        ...
        if shouldIgnoreCleanupAfterBlockedDictationStart && !isCancellingTranscription {
            fputs("[imla-native] ignoring dictation cancel because start was blocked\n", stderr)
            return
        }
        fputs("[imla-native] cancel\n", stderr)
        cancelHostedDictation()
        cancelInFlightDictationTranscription()
        resetDictationOutputMode()
    ...
    private func handleToggleStart(outputMode: DictationOutputMode? = nil) -> Bool {
        ...
        fputs("[imla-native] toggle dictation start\n", stderr)
        if dictationLatencyTraceID == nil {
        ...
        if isStreamingDictationBackend {
            if #available(macOS 15, *) {
                ...
                meetingMonitor.refreshState()
                fputs("[imla-native] Nemotron streaming toggle mode active\n", stderr)
    ...
    private func handleToggleStop() {
        fputs("[imla-native] toggle dictation stop\n", stderr)
        handleStop()
    }
    ...
    private func handleStop() {
        ...
        if shouldIgnoreCleanupAfterBlockedDictationStart {
            fputs("[imla-native] ignoring dictation stop because start was blocked\n", stderr)
            return
        }
        fputs("[imla-native] stop\n", stderr)
        let startedAt = dictationStartedAt ?? Date()
        dictationStartedAt = nil
        ...
        if isNemotron35Streaming {
            ...
            if #available(macOS 15, *), let controller = _streamingDictationController as? StreamingDictationController {
                ...
                fputs("[imla-native] Nemotron streaming stop, controller not ready (short press)\n", stderr)
        ...
        guard let sessionID = dictationAudioSessionManager.currentSessionID else {
            fputs("[imla-native] stop requested without an active audio session\n", stderr)
    ...
    private func cancelDictationAudioSessionForMeetingRecordingIfNeeded() {
        ...
        fputs("[imla-native] cancelling dictation audio session because meeting is active\n", stderr)
        cancelHostedDictation()
    ...
    private func finishNemotronStreamingStop(
        ...
        guard isNemotron35Streaming, nemotron35StreamingSessionID == sessionID else {
            fputs("[imla-native] ignoring stale Nemotron stop completion\n", stderr)
            return
        }
        isNemotron35Streaming = false
        _streamingDictationController = nil
        ...
        fputs("[imla-native] Nemotron streaming stop, got \(finalText.count) chars\n", stderr)
        ...
        Task { @MainActor [weak self] in
            ...
            fputs("[imla-native] Nemotron streaming done (\(String(format: "%.1f", duration))s)\n", stderr)
        ...
        do {
            ...
            if job.outputMode == .voiceNote {
                ...
                if canPasteDictation(to: job.correctionTargetApp) {
                    ...
                    fputs("[imla-native] dictation saved without paste because the target app changed\n", stderr)
                    applyDictationLifecycleActions(dictationLifecycleFeedback.finish(
                        sessionID: job.id,
                        outcome: .failure(
            ...
            fputs("[imla-native] test dictation cancelled\n", stderr)
            if let trace = job.latencyTrace {
                markDictationLatency("pipeline_cancelled", trace: trace)
            }
        } catch {
            ...
            fputs("[imla-native] transcription failed: \(error)\n", stderr)
            if job.isTestMode {
    ...
    /// frontmost right now.
    ///
    /// `canPasteDictation` substitutes `lastExternalApp` when Imla is frontmost,
    /// which is right for pasting but wrong for a send key: the key goes to whatever
    /// is actually focused, so that substitution could submit into Imla's own
    /// window. Imla never satisfies this check.
    ...
    private func userFacingDictationTestError(_ error: Error) -> String {
        ...
        if nsError.domain == "ImlaTranscriptionRuntime" {
            switch nsError.code {
            case 1:
        ...
        if lowercasedMessage.contains("permission") || lowercasedMessage.contains("microphone") {
            return "Imla could not access the microphone. Check Microphone permission and try again."
        }
    ...
    private func checkMaraudersMapActivation(_ text: String) {
        ...
        fputs("[imla-native] Marauder's Map unlocked!\n", stderr)
