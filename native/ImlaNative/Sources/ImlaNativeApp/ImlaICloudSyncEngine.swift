...
final class UserDefaultsICloudChangeTokenStore: ICloudChangeTokenStore {
    ...
    init(
        ...
        key: String = "imla.icloud.textRecords.ImlaSyncZone.serverChangeToken.v1"
    ) {
...
struct ImlaBridgeDeviceSnapshot: Equatable {
    let deviceID: String
    let platform: String
...
enum ImlaBridgeDeviceIdentity {
    ...
    static func local(defaults: UserDefaults = .standard) -> ImlaBridgeDeviceSnapshot {
        let deviceID: String
        ...
        return ImlaBridgeDeviceSnapshot(
            deviceID: deviceID,
            platform: "macOS",
    ...
    static func snapshot(from record: CKRecord) -> ImlaBridgeDeviceSnapshot? {
...
final class ImlaICloudSyncEngine {
    enum Schema {
        static let containerIdentifier = "iCloud.com.mueslihq.muesli"
        static let syncZoneName = "ImlaSyncZone"
        static let textRecordType = "ImlaTextRecord"
        static let bridgeDeviceRecordType = "ImlaBridgeDevice"
        static let textSubscriptionID = "muesli-text-records-sync-zone-v1"
        static let migratedDefaultZoneKey = "imla.icloud.textRecords.defaultToSyncZoneMigrated.v1"
    ...
    func refreshBridgeDeviceLink(forceRefresh: Bool = false) async {
        guard ImlaBridgeDeviceIdentity.shouldRefresh(
            defaults: defaults,
        ...
        do {
            ...
            ImlaBridgeDeviceIdentity.updateRemoteDevices(from: records, defaults: defaults)
            ImlaBridgeDeviceIdentity.markRefreshed(defaults: defaults)
        } catch {
            fputs(
                "Failed to refresh iCloud bridge device identity: \(String(describing: type(of: error)))\n",
                stderr
            )
            ImlaBridgeDeviceIdentity.markRefreshFailed(defaults: defaults)
        }
    }

    private func upsertLocalBridgeDeviceRecord() async throws {
        let snapshot = ImlaBridgeDeviceIdentity.local(defaults: defaults)
    ...
    private func fetchZoneChangesPage(
        ...
        try await ICloudSyncCallbackDeadline.wait { finish in
            ...
            operation.recordWithIDWasDeletedBlock = { _, _ in
                // Sync contract: Imla clients must delete text records by writing
