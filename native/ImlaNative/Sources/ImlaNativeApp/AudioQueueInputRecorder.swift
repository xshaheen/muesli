...
final class AudioQueueInputRecorder: StreamingDictationRecording, StreamingDictationLatencyReporting, PausableStreamingDictationRecording {
    ...
    private let processingQueue = DispatchQueue(label: "com.xshaheen.imla.audio-queue-input-recorder-processing")
    private let failureCallbackQueue = DispatchQueue(label: "com.xshaheen.imla.audio-queue-input-recorder-failures")
    ...
    init(directoryName: String = "imla-native-dictation", native: NativeOperations = NativeOperations()) {
        self.native = native
