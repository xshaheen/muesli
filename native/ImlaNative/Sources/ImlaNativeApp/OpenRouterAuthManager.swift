...
final class OpenRouterAuthManager {
    ...
    /// Removes only Imla's local credential. The user-controlled key remains
    ...
    func generateCallbackPath() -> String {
        "/imla/openrouter/oauth/\(randomBase64URL(byteCount: 32))"
    }
                    ...
                    Self.receiveHTTPRequest(on: connection) { request in
                        Task { @MainActor in
                            ...
                            case .authorizationCode(let code):
                                Self.sendHTTPResponse(
                                    status: 200,
                                    title: "OpenRouter connected to Imla",
                                    on: connection
                                )
                                timeout.cancel()
                                finish(.success(code))
