...
struct GoogleCalendarCredentials {
    ...
    /// Load credentials from app bundle (production) or ~/.config/imla/ (dev).
    ...
    static func load() -> GoogleCalendarCredentials? {
        ...
        let devPath = NSString("~/.config/imla/google-oauth.json").expandingTildeInPath
        let devURL = URL(fileURLWithPath: devPath)
        if let creds = parse(url: devURL) {
            fputs("[google-cal] credentials loaded from ~/.config/imla/\n", stderr)
            return creds
        }
