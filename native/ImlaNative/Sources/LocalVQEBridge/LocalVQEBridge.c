...
static void context_error(ImlaLocalVQEContext *context, const char *message) {
    if (context != NULL) {
...
ImlaLocalVQEContext *imla_localvqe_create(
    ...
    ImlaLocalVQEContext *context = (ImlaLocalVQEContext *)calloc(1, sizeof(ImlaLocalVQEContext));
    if (context == NULL) {
        free_context(localvqe);
        dlclose(library);
        set_error(error_buffer, error_buffer_length, "Could not allocate Imla LocalVQE context");
        return NULL;
    }
...
int imla_localvqe_process_frame_f32(
    ...
    int hop = imla_localvqe_hop_length(context);
    if (hop_samples != hop) {
