...
# Shared SwiftPM scratch-path resolution for local Imla builds.
#
...
#   3. ~/Library/Caches/imla-spm/<channel>
#
...
imla_resolve_spm_scratch_path() {
  ...
  printf '%s/%s\n' "$(imla_default_spm_cache_root)" "$channel"
}

imla_worktree_spm_scratch_channel() {
  local channel="${1:-dev}"
  local root="${2:-$PWD}"
  local root_name
  local root_hash
