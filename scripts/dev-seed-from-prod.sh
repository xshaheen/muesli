...
# Safely seed ImlaDev from production data without touching production files.
#
# Default behavior:
# - Reads from ~/Library/Application Support/Imla
# - Writes to ~/Library/Application Support/ImlaDev
# - Stages the current dev support dir first so unrelated dev-only files are preserved
# - Overlays production imla.db / sidecars and config.json into the staged temp directory
...
#   ./scripts/dev-seed-from-prod.sh --prod-dir "/path/to/Imla" --dev-dir "/path/to/ImlaDev"
#   ./scripts/dev-seed-from-prod.sh --dry-run

PROD_SUPPORT_DIR="${MUESLI_PROD_SUPPORT_DIR:-$HOME/Library/Application Support/Imla}"
DEV_SUPPORT_DIR="${MUESLI_DEV_SUPPORT_DIR:-$HOME/Library/Application Support/ImlaDev}"
COPY_CONFIG=1
DRY_RUN=0
FORCE=0
SWAPPED_OLD_DEV=0
...
running_processes() {
  ps ax -o pid=,comm= | awk '$2 == "Imla" || $2 == "ImlaDev" { print }'
}
...
PROD_DB="$PROD_SUPPORT_DIR/imla.db"
PROD_DB_SHM="$PROD_SUPPORT_DIR/imla.db-shm"
PROD_DB_WAL="$PROD_SUPPORT_DIR/imla.db-wal"
...
if [[ "$FORCE" -ne 1 ]]; then
  if running="$(running_processes)" && [[ -n "$running" ]]; then
    printf 'Refusing to seed while Imla or ImlaDev is running:\n%s\n' "$running" >&2
...
run_or_echo ditto "$PROD_DB" "$STAGE_DIR/imla.db"
if [[ -f "$PROD_DB_SHM" ]]; then
  run_or_echo ditto "$PROD_DB_SHM" "$STAGE_DIR/imla.db-shm"
fi
if [[ -f "$PROD_DB_WAL" ]]; then
  run_or_echo ditto "$PROD_DB_WAL" "$STAGE_DIR/imla.db-wal"
fi
...
if [[ "$DRY_RUN" -eq 0 ]]; then
  validate_sqlite_db "$STAGE_DIR/imla.db"
