...
#   - Production provisioning profile for com.xshaheen.imla when CloudKit
#     entitlements are enabled:
#       MUESLI_PROVISIONING_PROFILE=/path/to/profile.provisionprofile
#   - Notary profile stored: xcrun notarytool store-credentials ImlaNotary
...
if ! imla_spm_scratch_disabled; then
  SWIFTPM_SCRATCH_PATH="$(imla_resolve_spm_scratch_path release)"
...
APP_DIR="${MUESLI_RELEASE_APP_DIR:-$INSTALL_DIR/Imla.app}"
...
if [[ -z "$PROVISIONING_PROFILE" ]]; then
  ...
  echo "Use the production provisioning profile for bundle ID com.xshaheen.imla with CloudKit container iCloud.com.mueslihq.muesli." >&2
  exit 1
fi
...
DOWNLOAD_URL="https://github.com/xshaheen/muesli/releases/download/v${VERSION}/Imla-${VERSION}.dmg"
TAG="v${VERSION}"
RELEASE_TITLE="Imla ${VERSION}"
...
### Install
1. Download \`Imla-${VERSION}.dmg\`
2. Open the DMG and drag Imla to Applications
...
resume_existing_release_publication() {
  ...
  VERIFY_DIR=$(mktemp -d)
  local hosted_dmg="$VERIFY_DIR/Imla-${VERSION}.dmg"
...
echo "  Bundle ID: com.xshaheen.imla"
...
APP_ZIP="$OUTPUT_DIR/Imla-app-${VERSION}.zip"
...
DMG_PATH="$OUTPUT_DIR/Imla-${VERSION}.dmg"
...
SPCTL_RESULT=$(spctl -a -vv "$MOUNT_POINT/Imla.app" 2>&1)
echo "  $SPCTL_RESULT"

STAPLE_RESULT=$(xcrun stapler validate "$MOUNT_POINT/Imla.app" 2>&1)
echo "  $STAPLE_RESULT"

"$ROOT/scripts/verify_signed_cloud_entitlements.sh" \
  "$MOUNT_POINT/Imla.app" \
  Production \
  com.xshaheen.imla \
  iCloud.com.mueslihq.muesli \
  production
...
VERIFY_DIR=$(mktemp -d)
HOSTED_DMG="$VERIFY_DIR/Imla-${VERSION}.dmg"
...
HOSTED_APP_SPCTL_RESULT=$(spctl -a -vv "$HOSTED_MOUNT_POINT/Imla.app" 2>&1)
echo "  $HOSTED_APP_SPCTL_RESULT"

HOSTED_APP_STAPLE_RESULT=$(xcrun stapler validate "$HOSTED_MOUNT_POINT/Imla.app" 2>&1)
...
sed -i '' "s|https://github.com/xshaheen/muesli/releases/download/[^\"]*\\.dmg|$DOWNLOAD_URL|g" "$ROOT/docs/index.html"
sed -i '' "s|https://github.com/xshaheen/muesli/releases/download/.*\\.dmg|$DOWNLOAD_URL|g" "$ROOT/docs/llms.txt"
