...
- **Mixed-language repair:** Derived from the spoken-language selection, not a preset or a toggle. Two or more selected dictation languages compose a repair block into every cleanup prompt and turn the post-processor on once; two or more selected meeting languages run transcript cleanup on the meeting summary backend. Arabic+English gets worked examples, any other pair gets script-neutral text, and on-device cleanup gets a compact block sized for its 1,024-token context.
...
### Tests
```bash
swift test --package-path native/MuesliNative    # 2,808 @Test declarations across 249 suites
```
