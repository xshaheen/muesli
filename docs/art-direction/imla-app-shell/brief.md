# Imla app shell — Direction Brief

## Mode

Extend a settled direction. The Contextual Spark language shipped on the floating surfaces
(`imla-mini-indicator`, nodes 01–17) is the parent; this node carries it into the main
application window and reorganises settings around it.

## Invariant brief

- Audience: an existing Imla user who dictates all day and reviews meetings a few times a week.
...
- Imla source tree — `ImlaTheme.swift`, `FloatingIndicatorSurfaceStyle.swift`,
...
- `docs/art-direction/imla-mini-indicator/` brief and feedback history. Anchor: the settled
  Contextual Spark palette, geometry and motion, and the Mini's independent colour ownership.
- `docs/art-direction/imla-unified-glass/` brief. Anchor: the neutral-with-semantic-colour rule.
...
Imla's settings sprawled because it does three jobs (dictation, meetings, on-device model
