...
- User-provided Imla screenshots in this task: current meeting detail view and floating recording control. Anchor: actual density, geometry, and neutral dark appearance.
- Running `/Applications/ImlaDev.app` captured locally through macOS accessibility. Anchor: current 112 x 22 recording control.
- Repository source and `ImlaTheme`. Anchor: current tokens, 360 x 320 panel size, 20-point panel radius, and existing control inventory.
