...
- Keep: Imla's animated waveform as the recording signal.
...
- Intensify: visible computation through an animated field of points while Imla generates the final transcript.
- Soften: use Imla's warm accent rather than copying Monologue's cyan matrix.
...
- Keep: Imla's existing start cue when the microphone stream is actually active.
...
- Add: a three-second focus reminder — the Preparing seed appears beside a newly focused text caret to remind the user that dictation is available; one per focused field, 1.5 s cooldown, never inside Imla, never during a session, never announced; setting "Dictation reminder in text fields" (on by default).
