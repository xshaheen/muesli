...
Imla 0.8.4 adds voice-driven writing with Quill, more transcription choices, improved meeting workflows, and clearer macOS calendar setup. It also repairs private iCloud text sync for legacy libraries created by 0.8.2 or left unmatched after upgrading to 0.8.3.
...
- Uses calendars connected to macOS, including iCloud, Google, and Exchange accounts. Direct Google Calendar sign-in in Imla is unavailable for now.
...
- Refreshes account changes when returning to Imla, with background event queries that keep the interface responsive.
